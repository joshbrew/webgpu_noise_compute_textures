(()=>{var le=`<!-- tools/noise/noiseComponent.html -->\r
<div id="noise-app">\r
  <style>\r
    :root {\r
...
`;var se=`const PI : f32 = 3.141592653589793;\r
...
`;var fe=4096,ee=2048,Me=8,H=class{constructor(t,e){this.device=t,this.queue=e,this.entryPoints=["computePerlin","computeBillow","computeAntiBillow","computeRidge","computeAntiRidge","computeRidgedMultifractal","computeRidgedMultifractal2","computeRidgedMultifractal3","computeRidgedMultifractal4","computeAntiRidgedMultifractal","computeAntiRidgedMultifractal2","computeAntiRidgedMultifractal3","computeAntiRidgedMultifractal4","computeFBM","computeFBM2","computeFBM3","computeCellularBM1","computeCellularBM2","computeCellularBM3","computeVoronoiBM1","computeVoronoiBM2","computeVoronoiBM3","computeCellular","computeWorley","computeAntiCellular","computeAntiWorley","computeLanczosBillow","computeLanczosAntiBillow","computeVoronoiTileNoise","computeVoronoiCircleNoise","computeVoronoiCircle2","computeVoronoiFlatShade","computeVoronoiRipple3D","computeVoronoiRipple3D2","computeVoronoiCircularRipple","computeFVoronoiRipple3D","computeFVoronoiCircularRipple","computeRippleNoise","computeFractalRipples","computeHexWorms","computePerlinWorms","computeWhiteNoise","computeBlueNoise","computeSimplex","computeSimplexFBM","computeCurl2D","computeCurlFBM2D","computeDomainWarpFBM1","computeDomainWarpFBM2","computeGaborAnisotropic","computeTerraceNoise","computeFoamNoise","computeTurbulence","computePerlin4D","computeWorley4D","computeAntiWorley4D","computeCellular4D","computeAntiCellular4D","computeBillow4D","computeAntiBillow4D","computeLanczosBillow4D","computeLanczosAntiBillow4D","computeFBM4D","computeVoronoi4D","computeVoronoiBM1_4D","computeVoronoiBM2_4D","computeVoronoiBM3_4D","computeVoronoiBM1_4D_vec","computeVoronoiBM2_4D_vec","computeVoronoiBM3_4D_vec","computeWorleyBM1_4D","computeWorleyBM2_4D","computeWorleyBM3_4D","computeWorleyBM1_4D_vec","computeWorleyBM2_4D_vec","computeWorleyBM3_4D_vec","computeCellularBM1_4D","computeCellularBM2_4D","computeCellularBM3_4D","computeCellularBM1_4D_vec","computeCellularBM2_4D_vec","computeCellularBM3_4D_vec","computeTerraceNoise4D","computeFoamNoise4D","computeTurbulence4D","computeGauss5x5","computeNormal","computeNormal8","computeSphereNormal","computeNormalVolume","clearTexture"],this.shaderModule=t.createShaderModule({code:se}),this.bindGroupLayout=t.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}},{binding:1,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}},{binding:2,visibility:GPUShaderStage.COMPUTE,buffer:{type:"read-only-storage"}},{binding:3,visibility:GPUShaderStage.COMPUTE,texture:{sampleType:"float",viewDimension:"2d-array"}},{binding:4,visibility:GPUShaderStage.COMPUTE,storageTexture:{access:"write-only",format:"rgba16float",viewDimension:"2d-array"}},{binding:5,visibility:GPUShaderStage.COMPUTE,buffer:{type:"read-only-storage"}},{binding:6,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}},{binding:7,visibility:GPUShaderStage.COMPUTE,texture:{sampleType:"float",viewDimension:"3d"}},{binding:8,visibility:GPUShaderStage.COMPUTE,storageTexture:{access:"write-only",format:"rgba16float",viewDimension:"3d"}}]}),this.pipelineLayout=t.createPipelineLayout({bindGroupLayouts:[this.bindGroupLayout]}),this.pipelines=new Map,this._texPairs=new Map,this._tid=null,this._tag=new WeakMap,this._volumeCache=new Map,this.viewA=null,this.viewB=null,this.width=0,this.height=0,this.layers=1,this.isA=!0,this._initBuffers(),this._ensureDummies(),this._ctxMap=new WeakMap}_initBuffers(){this.optionsBuffer?.destroy(),this.paramsBuffer?.destroy(),this.permBuffer?.destroy(),this.nullPosBuffer?.destroy(),this.optionsBuffer=this.device.createBuffer({size:32,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),this.paramsBuffer=this.device.createBuffer({size:88,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),this.permBuffer=this.device.createBuffer({size:512*4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}),this.nullPosBuffer=this.device.createBuffer({size:64,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}),this.queue.writeBuffer(this.optionsBuffer,0,new ArrayBuffer(32)),this.queue.writeBuffer(this.paramsBuffer,0,new ArrayBuffer(88)),this.queue.writeBuffer(this.permBuffer,0,new Uint32Array(512))}_ensureDummies(){this._dummy2D_sampleTex||(this._dummy2D_sampleTex=this.device.createTexture({size:[1,1,1],format:"rgba16float",usage:GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_SRC}),this._dummy2D_sampleView=this._dummy2D_sampleTex.createView({dimension:"2d-array",arrayLayerCount:1})),this._dummy2D_writeTex||(this._dummy2D_writeTex=this.device.createTexture({size:[1,1,1],format:"rgba16float",usage:GPUTextureUsage.STORAGE_BINDING|GPUTextureUsage.COPY_DST}),this._dummy2D_writeView=this._dummy2D_writeTex.createView({dimension:"2d-array",arrayLayerCount:1})),this._dummy3D_sampleTex||(this._dummy3D_sampleTex=this.device.createTexture({size:{width:1,height:1,depthOrArrayLayers:1},dimension:"3d",format:"rgba16float",usage:GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_SRC}),this._dummy3D_sampleView=this._dummy3D_sampleTex.createView({dimension:"3d"})),this._dummy3D_writeTex||(this._dummy3D_writeTex=this.device.createTexture({size:{width:1,height:1,depthOrArrayLayers:1},dimension:"3d",format:"rgba16float",usage:GPUTextureUsage.STORAGE_BINDING|GPUTextureUsage.COPY_DST}),this._dummy3D_writeView=this._dummy3D_writeTex.createView({dimension:"3d"}))}resize(t){this.maxConfigs=t,this._initBuffers()}setPermTable(t){this.queue.writeBuffer(this.permBuffer,0,t)}setPosBuffer(t){this.posBuffer=t}setInputTextureView(t){try{if(((t?.texture?.usage??0)&GPUTextureUsage.TEXTURE_BINDING)===0){console.warn("setInputTextureView: provided texture view not created with TEXTURE_BINDING; ignoring.");return}}catch{}if(this.inputTextureView=t,this._tid!==null){let e=this._texPairs.get(this._tid);e&&(e.bindGroupDirty=!0)}}setOutputTextureView(t){try{if(((t?.texture?.usage??0)&GPUTextureUsage.STORAGE_BINDING)===0){console.warn("setOutputTextureView: provided texture view not created with STORAGE_BINDING; ignoring.");return}}catch{}if(this.outputTextureView=t,this._tid!==null){let e=this._texPairs.get(this._tid);e&&(e.bindGroupDirty=!0)}}buildPermTable(t=Date.now()){let i=new te(t).perm,r=new Uint32Array(512);for(let a=0;a<512;a++)r[a]=i[a];this.setPermTable(r)}setOptions(t={}){Array.isArray(t.noiseChoices)?this.noiseChoices=t.noiseChoices:this.noiseChoices||(this.noiseChoices=[0]);let{getGradient:e=0,outputChannel:i=1,baseRadius:r=0,heightScale:a=1,useCustomPos:o=0,ioFlags:s=0}=t;this.useCustomPos=o>>>0;let l=new ArrayBuffer(32),f=new DataView(l);f.setUint32(0,e,!0),f.setUint32(4,this.useCustomPos,!0),f.setUint32(8,i,!0),f.setUint32(12,s>>>0,!0),f.setFloat32(16,r,!0),f.setFloat32(20,a,!0),f.setFloat32(24,0,!0),f.setFloat32(28,0,!0),this.queue.writeBuffer(this.optionsBuffer,0,l);for(let c of this._texPairs.values())c.bindGroupDirty=!0}setNoiseParams(t={}){let{seed:e=Date.now()|0,zoom:i=1,freq:r=1,octaves:a=8,lacunarity:o=2,gain:s=.5,xShift:l=0,yShift:f=0,zShift:c=0,turbulence:u=0,seedAngle:m=0,exp1:x=1,exp2:g=0,threshold:d=.1,rippleFreq:v=10,time:b=0,warpAmp:D=.5,gaborRadius:w=4,terraceStep:M=8,toroidal:_=0,voroMode:P=0,edgeK:T=0}=t,A=Math.max(Number(i)||0,1e-6),C=Math.max(Number(r)||0,1e-6),B=Math.max(0,Math.floor(Number(a)||0))>>>0,R=(u?1:0)>>>0,S=(_?1:0)>>>0,q=new ArrayBuffer(88),y=new DataView(q),z=0;y.setUint32(z+0,e>>>0,!0),y.setFloat32(z+4,A,!0),y.setFloat32(z+8,C,!0),y.setUint32(z+12,B,!0),y.setFloat32(z+16,o,!0),y.setFloat32(z+20,s,!0),y.setFloat32(z+24,l,!0),y.setFloat32(z+28,f,!0),y.setFloat32(z+32,c,!0),y.setUint32(z+36,R,!0),y.setFloat32(z+40,m,!0),y.setFloat32(z+44,x,!0),y.setFloat32(z+48,g,!0),y.setFloat32(z+52,d,!0),y.setFloat32(z+56,v,!0),y.setFloat32(z+60,b,!0),y.setFloat32(z+64,D,!0),y.setFloat32(z+68,w,!0),y.setFloat32(z+72,M,!0),y.setUint32(z+76,S,!0),y.setUint32(z+80,P>>>0,!0),y.setFloat32(z+84,T,!0),this.queue.writeBuffer(this.paramsBuffer,0,q);for(let p of this._texPairs.values())p.bindGroupDirty=!0;for(let[p,h]of this._volumeCache)!h||!Array.isArray(h.chunks)||(h._bindGroupsDirty=!0)}_compute2DTiling(t,e){let i=Math.min(t,fe),r=Math.min(e,fe),a=Math.ceil(t/i),o=Math.ceil(e/r),s=a*o;return{tileW:i,tileH:r,tilesX:a,tilesY:o,layers:s}}_create2DPair(t,e){let i=this._compute2DTiling(t,e),r=GPUTextureUsage.STORAGE_BINDING|GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_SRC|GPUTextureUsage.COPY_DST,a=()=>this.device.createTexture({size:[i.tileW,i.tileH,i.layers],format:"rgba16float",usage:r}),o={dimension:"2d-array",arrayLayerCount:i.layers},s=a(),l=a(),f=s.createView(o),c=l.createView(o);s.label=`2D texA ${t}x${e}x${i.layers}`,l.label=`2D texB ${t}x${e}x${i.layers}`,f.label="2D:viewA",c.label="2D:viewB",this._tag.set(f,"2D:A"),this._tag.set(c,"2D:B");let u=this._texPairs.size;return this._texPairs.set(u,{texA:s,texB:l,viewA:f,viewB:c,fullWidth:t,fullHeight:e,tileWidth:i.tileW,tileHeight:i.tileH,tilesX:i.tilesX,tilesY:i.tilesY,layers:i.layers,isA:!0,tiles:null,bindGroupDirty:!0}),this._tid===null&&this.setActiveTexture(u),u}createShaderTextures(t,e){this._tid!==null&&this._texPairs.has(this._tid)&&this.destroyTexturePair(this._tid);let i=this._create2DPair(t,e);return this.setActiveTexture(i),i}destroyTexturePair(t){let e=this._texPairs.get(t);if(e){try{e.texA.destroy()}catch{}try{e.texB.destroy()}catch{}if(Array.isArray(e.tiles))for(let i of e.tiles){if(Array.isArray(i.frames))for(let r of i.frames)try{r.destroy()}catch{}if(i.posBuf&&i.posBuf!==this.nullPosBuffer)try{i.posBuf.destroy()}catch{}}this._texPairs.delete(t),this._tid===t&&(this._tid=null,this.inputTextureView=null,this.outputTextureView=null,this.viewA=null,this.viewB=null)}}destroyAllTexturePairs(){let t=Array.from(this._texPairs.keys());for(let e of t)this.destroyTexturePair(e)}setActiveTexture(t){if(!this._texPairs.has(t))throw new Error("setActiveTexture: invalid id");this._tid=t;let e=this._texPairs.get(t);this.viewA=e.viewA,this.viewB=e.viewB,this.width=e.tileWidth,this.height=e.tileHeight,this.layers=e.layers,this.inputTextureView=e.isA?e.viewA:e.viewB,this.outputTextureView=e.isA?e.viewB:e.viewA}_buildPosBuffer(t,e,i){if((this.useCustomPos|0)===0&&!i)return this.nullPosBuffer;let r=t*e,a=i instanceof Float32Array&&i.length===r*4?i:new Float32Array(r*4),o=this.device.createBuffer({size:a.byteLength,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST});return this.queue.writeBuffer(o,0,a.buffer,a.byteOffset,a.byteLength),o}_writeFrameUniform(t,e){let i=new ArrayBuffer(64),r=new DataView(i);r.setUint32(0,e.fullWidth>>>0,!0),r.setUint32(4,e.fullHeight>>>0,!0),r.setUint32(8,e.tileWidth>>>0,!0),r.setUint32(12,e.tileHeight>>>0,!0),r.setInt32(16,e.originX|0,!0),r.setInt32(20,e.originY|0,!0),r.setInt32(24,e.originZ|0,!0),r.setUint32(28,e.fullDepth>>>0,!0),r.setUint32(32,e.tileDepth>>>0,!0),r.setInt32(36,e.layerIndex|0,!0),r.setUint32(40,e.layers>>>0,!0),r.setUint32(44,0,!0),r.setFloat32(48,e.originXf??0,!0),r.setFloat32(52,e.originYf??0,!0),r.setFloat32(56,0,!0),r.setFloat32(60,0,!0),this.queue.writeBuffer(t,0,i)}_create2DTileBindGroups(t,e={}){let i=this._texPairs.get(t);if(!i)throw new Error("_create2DTileBindGroups: invalid tid");if(Array.isArray(i.tiles)&&!i.bindGroupDirty&&!e.customData)return;let r=[];for(let a=0;a<i.tilesY;a++)for(let o=0;o<i.tilesX;o++){let s=a*i.tilesX+o,l=o*i.tileWidth,f=a*i.tileHeight,c=i.tiles&&i.tiles[s]||null,u;c&&c.posBuf&&!e.customData?u=c.posBuf:u=this._buildPosBuffer(i.tileWidth,i.tileHeight,e.customData);let m;c&&c.frames&&c.frames[0]?m=c.frames[0]:m=this.device.createBuffer({size:64,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});let x=Number.isFinite(e.frameFullWidth)?e.frameFullWidth>>>0:i.fullWidth,g=Number.isFinite(e.frameFullHeight)?e.frameFullHeight>>>0:i.fullHeight,d=x/i.fullWidth,v=g/i.fullHeight,b=l*d,D=f*v;this._writeFrameUniform(m,{fullWidth:x,fullHeight:g,tileWidth:i.tileWidth,tileHeight:i.tileHeight,originX:l,originY:f,originZ:0,fullDepth:1,tileDepth:1,layerIndex:s,layers:i.layers,originXf:b,originYf:D});let w=c?.bgs?.[0]?.bgA??null,M=c?.bgs?.[0]?.bgB??null;if(!w||!M||i.bindGroupDirty)try{w=this.device.createBindGroup({layout:this.bindGroupLayout,entries:[{binding:0,resource:{buffer:this.optionsBuffer}},{binding:1,resource:{buffer:this.paramsBuffer}},{binding:2,resource:{buffer:this.permBuffer}},{binding:3,resource:i.viewA},{binding:4,resource:i.viewB},{binding:5,resource:{buffer:u}},{binding:6,resource:{buffer:m}},{binding:7,resource:this._dummy3D_sampleView},{binding:8,resource:this._dummy3D_writeView}]}),M=this.device.createBindGroup({layout:this.bindGroupLayout,entries:[{binding:0,resource:{buffer:this.optionsBuffer}},{binding:1,resource:{buffer:this.paramsBuffer}},{binding:2,resource:{buffer:this.permBuffer}},{binding:3,resource:i.viewB},{binding:4,resource:i.viewA},{binding:5,resource:{buffer:u}},{binding:6,resource:{buffer:m}},{binding:7,resource:this._dummy3D_sampleView},{binding:8,resource:this._dummy3D_writeView}]})}catch(_){throw new Error(`_create2DTileBindGroups: createBindGroup failed: ${_?.message||_}`)}r.push({layerIndex:s,originX:l,originY:f,frames:[m],posBuf:u,bgs:[{bgA:w,bgB:M}]})}i.tiles=r,i.bindGroupDirty=!1,this._tid===t&&(this._tiles=r)}async _runPipelines(t,e,i,r,a,o,s=1){let l=t,f=e,c=Array.isArray(o),u=0,m=this.device.createCommandEncoder(),x=m.beginComputePass();for(let g of this.noiseChoices){let d=typeof g=="number"?this.entryPoints[g]:g,v=this.pipelines.get(d);v||(v=this.device.createComputePipeline({layout:this.pipelineLayout,compute:{module:this.shaderModule,entryPoint:d}}),this.pipelines.set(d,v)),c&&this.setNoiseParams(o[u++]),x.setPipeline(v),x.setBindGroup(0,l),x.dispatchWorkgroups(Math.ceil(i/8),Math.ceil(r/8),s),[l,f]=[f,l]}return x.end(),this.queue.submit([m.finish()]),f}async computeToTexture(t,e,i={},r={}){let a=t|0,o=e|0;if(!(a>0&&o>0))throw new Error(`computeToTexture: invalid size ${t}x${e}`);this._tid==null&&this._create2DPair(a,o);let s=this._texPairs.get(this._tid);if(!s||s.fullWidth!==a||s.fullHeight!==o){let x=this._create2DPair(a,o);this.setActiveTexture(x),s=this._texPairs.get(x)}i&&!Array.isArray(i)&&this.setNoiseParams(i);let l=r||{};this.setOptions({...l,ioFlags:0,useCustomPos:l.useCustomPos??this.useCustomPos}),(!s.tiles||s.bindGroupDirty||l.customData)&&this._create2DTileBindGroups(this._tid,r);let f=s.isA,c=null,u=null;for(let x of s.tiles){let{bgs:g}=x,{bgA:d,bgB:v}=g[0],b=c?c===d?d:v:f?d:v,D=b===d?v:d;c=await this._runPipelines(b,D,s.tileWidth,s.tileHeight,1,i,1),u={bgA:d,bgB:v}}let m=c===u.bgB;return s.isA=m,this.isA=m,this.setActiveTexture(this._tid),this.getCurrentView()}getCurrentView(){let t=this._texPairs.get(this._tid);return t?t.isA?t.viewA:t.viewB:null}_compute3DTiling(t,e,i){let r=Math.min(t,ee),a=Math.min(e,ee),o=this.device?.limits?.maxBufferSize??256*1024*1024,s=r*a*Me,l=Math.max(1,Math.floor(o*.8/Math.max(1,s))),f=Math.min(i,ee,l),c=Math.ceil(t/r),u=Math.ceil(e/a),m=Math.ceil(i/f);return{tw:r,th:a,td:f,nx:c,ny:u,nz:m}}_create3DChunks(t,e,i){let r=this._compute3DTiling(t,e,i),a=[],o=GPUTextureUsage.STORAGE_BINDING|GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_SRC|GPUTextureUsage.COPY_DST;for(let s=0;s<r.nz;s++)for(let l=0;l<r.ny;l++)for(let f=0;f<r.nx;f++){let c=f*r.tw,u=l*r.th,m=s*r.td,x=this.device.createTexture({size:{width:r.tw,height:r.th,depthOrArrayLayers:r.td},dimension:"3d",format:"rgba16float",usage:o}),g=this.device.createTexture({size:{width:r.tw,height:r.th,depthOrArrayLayers:r.td},dimension:"3d",format:"rgba16float",usage:o}),d=x.createView({dimension:"3d"}),v=g.createView({dimension:"3d"});x.label=`3D texA ${r.tw}x${r.th}x${r.td} @ (${f},${l},${s})`,g.label=`3D texB ${r.tw}x${r.th}x${r.td} @ (${f},${l},${s})`,d.label=`3D:viewA[${f},${l},${s}]`,v.label=`3D:viewB[${f},${l},${s}]`,this._tag.set(d,`3D:A[${f},${l},${s}]`),this._tag.set(v,`3D:B[${f},${l},${s}]`),a.push({texA:x,texB:g,viewA:d,viewB:v,ox:c,oy:u,oz:m,w:r.tw,h:r.th,d:r.td,isA:!0,fb:null,posBuf:null,bgA:null,bgB:null})}return{chunks:a,tile:{w:r.tw,h:r.th,d:r.td},full:{w:t,h:e,d:i},grid:{nx:r.nx,ny:r.ny,nz:r.nz}}}_destroy3DSet(t){if(t)for(let e of t.chunks){try{e.texA.destroy()}catch{}try{e.texB.destroy()}catch{}if(e.viewA=null,e.viewB=null,e.bgA=null,e.bgB=null,e.fb){try{e.fb.destroy()}catch{}e.fb=null}if(e.posBuf&&e.posBuf!==this.nullPosBuffer){try{e.posBuf.destroy()}catch{}e.posBuf=null}}}destroyAllVolumes(){for(let[t,e]of this._volumeCache)this._destroy3DSet(e),this._volumeCache.delete(t)}get3DView(t){let e=this._volumeCache.get(String(t));if(!e)return null;let i=e.chunks.map(r=>r.isA?r.viewA:r.viewB);return i.length===1?i[0]:{views:i,meta:{full:e.full,tile:e.tile,grid:e.grid}}}destroyVolume(t){let e=String(t),i=this._volumeCache.get(e);i&&(this._destroy3DSet(i),this._volumeCache.delete(e))}_getOrCreate3DVolume(t,e,i,r=null,a=null){let o=r?String(r):`${t}x${e}x${i}`,s=this._volumeCache.get(o);if(s)return s;s=this._create3DChunks(t,e,i);for(let l of s.chunks){l.fb=this.device.createBuffer({size:64,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});let f=a&&Number.isFinite(a?.w)?a.w>>>0:s.full.w,c=a&&Number.isFinite(a?.h)?a.h>>>0:s.full.h,u=a&&Number.isFinite(a?.d)?a.d>>>0:s.full.d,m=f/s.full.w,x=c/s.full.h,g=l.ox*m,d=l.oy*x;this._writeFrameUniform(l.fb,{fullWidth:f,fullHeight:c,tileWidth:l.w,tileHeight:l.h,originX:l.ox,originY:l.oy,originZ:l.oz,fullDepth:u,tileDepth:l.d,layerIndex:0,layers:1,originXf:g,originYf:d});let v=this._buildPosBuffer(l.w,l.h,null);l.posBuf=v;try{l.bgA=this.device.createBindGroup({layout:this.bindGroupLayout,entries:[{binding:0,resource:{buffer:this.optionsBuffer}},{binding:1,resource:{buffer:this.paramsBuffer}},{binding:2,resource:{buffer:this.permBuffer}},{binding:3,resource:this._dummy2D_sampleView},{binding:4,resource:this._dummy2D_writeView},{binding:5,resource:{buffer:v}},{binding:6,resource:{buffer:l.fb}},{binding:7,resource:l.viewA},{binding:8,resource:l.viewB}]}),l.bgB=this.device.createBindGroup({layout:this.bindGroupLayout,entries:[{binding:0,resource:{buffer:this.optionsBuffer}},{binding:1,resource:{buffer:this.paramsBuffer}},{binding:2,resource:{buffer:this.permBuffer}},{binding:3,resource:this._dummy2D_sampleView},{binding:4,resource:this._dummy2D_writeView},{binding:5,resource:{buffer:l.posBuf}},{binding:6,resource:{buffer:l.fb}},{binding:7,resource:l.viewB},{binding:8,resource:l.viewA}]})}catch(b){throw new Error(`_getOrCreate3DVolume: createBindGroup failed: ${b?.message||b}`)}}return s._bindGroupsDirty=!1,this._volumeCache.set(o,s),s}_recreate3DBindGroups(t,e=null){if(!t||!Array.isArray(t.chunks))return;let i=e&&Number.isFinite(e.w)?e.w>>>0:t.full.w,r=e&&Number.isFinite(e.h)?e.h>>>0:t.full.h,a=e&&Number.isFinite(e.d)?e.d>>>0:t.full.d;for(let o of t.chunks){if(!o.fb){o.fb=this.device.createBuffer({size:64,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});let f=i/t.full.w,c=r/t.full.h,u=o.ox*f,m=o.oy*c;this._writeFrameUniform(o.fb,{fullWidth:i,fullHeight:r,tileWidth:o.w,tileHeight:o.h,originX:o.ox,originY:o.oy,originZ:o.oz,fullDepth:a,tileDepth:o.d,layerIndex:0,layers:1,originXf:u,originYf:m})}o.posBuf||(o.posBuf=this._buildPosBuffer(o.w,o.h,null));let s=[{binding:0,resource:{buffer:this.optionsBuffer}},{binding:1,resource:{buffer:this.paramsBuffer}},{binding:2,resource:{buffer:this.permBuffer}},{binding:3,resource:this._dummy2D_sampleView},{binding:4,resource:this._dummy2D_writeView},{binding:5,resource:{buffer:o.posBuf}},{binding:6,resource:{buffer:o.fb}},{binding:7,resource:o.viewA},{binding:8,resource:o.viewB}],l=[{binding:0,resource:{buffer:this.optionsBuffer}},{binding:1,resource:{buffer:this.paramsBuffer}},{binding:2,resource:{buffer:this.permBuffer}},{binding:3,resource:this._dummy2D_sampleView},{binding:4,resource:this._dummy2D_writeView},{binding:5,resource:{buffer:o.posBuf}},{binding:6,resource:{buffer:o.fb}},{binding:7,resource:o.viewB},{binding:8,resource:o.viewA}];try{o.bgA=this.device.createBindGroup({layout:this.bindGroupLayout,entries:s}),o.bgB=this.device.createBindGroup({layout:this.bindGroupLayout,entries:l})}catch(f){throw new Error(`_recreate3DBindGroups: failed to create bind groups: ${f?.message||f}`)}}t._bindGroupsDirty=!1}async computeToTexture3D(t,e,i,r={},a={}){let o=t|0,s=e|0,l=i|0;if(!(o>0&&s>0&&l>0))throw new Error(`computeToTexture3D: invalid size ${t}x${e}x${i}`);r&&!Array.isArray(r)&&this.setNoiseParams(r);let f=a||{};this.setOptions({...f,ioFlags:3,useCustomPos:f.useCustomPos??this.useCustomPos});let c=a&&(Number.isFinite(a.frameFullWidth)||Number.isFinite(a.frameFullHeight)||Number.isFinite(a.frameFullDepth))?{w:Number.isFinite(a.frameFullWidth)?a.frameFullWidth>>>0:o,h:Number.isFinite(a.frameFullHeight)?a.frameFullHeight>>>0:s,d:Number.isFinite(a.frameFullDepth)?a.frameFullDepth>>>0:l}:null,u=this._getOrCreate3DVolume(o,s,l,a.id,c);if(!u)throw new Error("computeToTexture3D: failed to create or retrieve volume");(u._bindGroupsDirty||!u.chunks[0].bgA||!u.chunks[0].bgB)&&this._recreate3DBindGroups(u,c);let m=null;for(let g of u.chunks){let d=g.isA?g.bgA:g.bgB,v=g.isA?g.bgB:g.bgA;if(!d||!v)throw new Error("computeToTexture3D: missing bind groups (volume not initialized correctly)");m=await this._runPipelines(d,v,g.w,g.h,g.d,r,g.d),g.isA=m===g.bgB}let x=u.chunks.map(g=>g.isA?g.viewA:g.viewB);return x.length===1?x[0]:{views:x,meta:{full:u.full,tile:u.tile,grid:u.grid}}}configureCanvas(t){let e=navigator.gpu.getPreferredCanvasFormat&&navigator.gpu.getPreferredCanvasFormat()||"bgra8unorm",i=t.getContext("webgpu");i.configure({device:this.device,format:e,alphaMode:"opaque",size:[t.width,t.height]}),this._ctxMap.set(t,{ctx:i,size:[t.width,t.height]})}initBlitRender(){this.sampler||(this.sampler=this.device.createSampler({magFilter:"linear",minFilter:"linear",addressModeU:"clamp-to-edge",addressModeV:"clamp-to-edge"})),this.bgl2D||(this.bgl2D=this.device.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.FRAGMENT,sampler:{}},{binding:1,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"float",viewDimension:"2d-array"}},{binding:2,visibility:GPUShaderStage.FRAGMENT,buffer:{type:"uniform"}}]}),this.pipeline2D=this.device.createRenderPipeline({layout:this.device.createPipelineLayout({bindGroupLayouts:[this.bgl2D]}),vertex:{module:this.device.createShaderModule({code:Q}),entryPoint:"vs_main"},fragment:{module:this.device.createShaderModule({code:Q}),entryPoint:"fs_main",targets:[{format:"bgra8unorm"}]},primitive:{topology:"triangle-list"}}),this.blit2DUbo=this.device.createBuffer({size:16,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST})),this.bgl3D||(this.bgl3D=this.device.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.FRAGMENT,sampler:{}},{binding:1,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"float",viewDimension:"3d"}},{binding:2,visibility:GPUShaderStage.FRAGMENT,buffer:{type:"uniform"}}]}),this.pipeline3D=this.device.createRenderPipeline({layout:this.device.createPipelineLayout({bindGroupLayouts:[this.bgl3D]}),vertex:{module:this.device.createShaderModule({code:J}),entryPoint:"vs_main"},fragment:{module:this.device.createShaderModule({code:J}),entryPoint:"fs_main",targets:[{format:"bgra8unorm"}]},primitive:{topology:"triangle-list"}}),this.blit3DUbo=this.device.createBuffer({size:16,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}))}_renderCommonCanvasSetup(t,e){let i="bgra8unorm",r=this._ctxMap.get(t);if(r){let s=t.width|0,l=t.height|0;(r.size[0]!==s||r.size[1]!==l)&&(r.size=[s,l],r.ctx.configure({device:this.device,format:i,alphaMode:"opaque",size:r.size}))}else{let s=t.getContext("webgpu"),l=[t.width|0,t.height|0];s.configure({device:this.device,format:i,alphaMode:"opaque",size:l}),r={ctx:s,size:l},this._ctxMap.set(t,r)}let a=this.device.createCommandEncoder(),o=a.beginRenderPass({colorAttachments:[{view:r.ctx.getCurrentTexture().createView(),loadOp:e?"clear":"load",clearValue:{r:0,g:0,b:0,a:1},storeOp:"store"}]});return{enc:a,pass:o,ctxEntry:r}}renderTextureToCanvas(t,e,i={}){let{layer:r=0,channel:a=0,preserveCanvasSize:o=!0,clear:s=!0}=i;if(this.initBlitRender(),!o)try{let m=t.texture;m&&typeof m.width=="number"&&typeof m.height=="number"&&(e.width=m.width,e.height=m.height)}catch{}let l=new Uint32Array([r>>>0,a>>>0,0,0]);this.queue.writeBuffer(this.blit2DUbo,0,l.buffer,l.byteOffset,l.byteLength);let f=this.device.createBindGroup({layout:this.bgl2D,entries:[{binding:0,resource:this.sampler},{binding:1,resource:t},{binding:2,resource:{buffer:this.blit2DUbo}}]}),{enc:c,pass:u}=this._renderCommonCanvasSetup(e,s);u.setPipeline(this.pipeline2D),u.setBindGroup(0,f),u.draw(6,1,0,0),u.end(),this.queue.submit([c.finish()])}renderTexture3DSliceToCanvas(t,e,i={}){let{depth:r,slice:a=0,zNorm:o=null,channel:s=0,chunk:l=0,preserveCanvasSize:f=!0,clear:c=!0}=i;this.initBlitRender();let u,m;if(t&&t.views&&Array.isArray(t.views)?(u=t.views[Math.max(0,Math.min(l|0,t.views.length-1))],m=t.meta?.tile?.d??r):(u=t,m=r),!u||!m)throw new Error("renderTexture3DSliceToCanvas: need a 3D view and its depth");if(!f)try{let w=u.texture;w&&typeof w.width=="number"&&typeof w.height=="number"&&(e.width=w.width,e.height=w.height)}catch{}let x=o??(Math.min(Math.max(a,0),m-1)+.5)/m;x=Math.min(Math.max(x,0),1);let g=new ArrayBuffer(16),d=new DataView(g);d.setFloat32(0,x,!0),d.setUint32(4,s>>>0,!0),d.setUint32(8,0,!0),d.setUint32(12,0,!0),this.queue.writeBuffer(this.blit3DUbo,0,g);let v=this.device.createBindGroup({layout:this.bgl3D,entries:[{binding:0,resource:this.sampler},{binding:1,resource:u},{binding:2,resource:{buffer:this.blit3DUbo}}]}),{enc:b,pass:D}=this._renderCommonCanvasSetup(e,c);D.setPipeline(this.pipeline3D),D.setBindGroup(0,v),D.draw(6,1,0,0),D.end(),this.queue.submit([b.finish()])}setExportBackground(t="black"){this.exportBackground=t}_resolveExportBackground(t){let e=t===void 0?this.exportBackground:t;if(e==null)return{r:0,g:0,b:0,a:1,transparent:!1};if(typeof e=="string"){let r=e.trim().toLowerCase();if(r==="transparent")return{r:0,g:0,b:0,a:0,transparent:!0};if(r==="black")return{r:0,g:0,b:0,a:1,transparent:!1};if(r==="white")return{r:1,g:1,b:1,a:1,transparent:!1};if(r[0]==="#")return this._parseHexBackground(r)}let i=r=>{let a=Number(r);if(!Number.isFinite(a))return 0;let o=a>1?a/255:a;return Math.min(Math.max(o,0),1)};if(Array.isArray(e)){let r=i(e[0]),a=i(e[1]),o=i(e[2]),s=e.length>=4?i(e[3]):1;return{r,g:a,b:o,a:s,transparent:s<=0}}if(typeof e=="object"){let r=i(e.r),a=i(e.g),o=i(e.b),s=e.a===void 0?1:i(e.a);return{r,g:a,b:o,a:s,transparent:s<=0}}return{r:0,g:0,b:0,a:1,transparent:!1}}_parseHexBackground(t){let e=String(t).trim().replace(/^#/,""),i=m=>m+m,r=0,a=0,o=0,s=255;if(e.length===3||e.length===4)r=parseInt(i(e[0]),16),a=parseInt(i(e[1]),16),o=parseInt(i(e[2]),16),e.length===4&&(s=parseInt(i(e[3]),16));else if(e.length===6||e.length===8)r=parseInt(e.slice(0,2),16),a=parseInt(e.slice(2,4),16),o=parseInt(e.slice(4,6),16),e.length===8&&(s=parseInt(e.slice(6,8),16));else return{r:0,g:0,b:0,a:1,transparent:!1};let l=r/255,f=a/255,c=o/255,u=s/255;return{r:l,g:f,b:c,a:u,transparent:u<=0}}_applyExportBackground(t,e){if(!t||!e||e.transparent)return;let i=Math.round(e.r*255),r=Math.round(e.g*255),a=Math.round(e.b*255),o=Math.round((e.a??1)*255);if(o<=0)return;let s=t.length|0;if(o>=255){for(let l=0;l<s;l+=4){let f=t[l+3]|0;if(f===255)continue;if(f===0){t[l+0]=i,t[l+1]=r,t[l+2]=a,t[l+3]=255;continue}let c=255-f;t[l+0]=(t[l+0]*f+i*c)/255|0,t[l+1]=(t[l+1]*f+r*c)/255|0,t[l+2]=(t[l+2]*f+a*c)/255|0,t[l+3]=255}return}for(let l=0;l<s;l+=4){let f=t[l+0]|0,c=t[l+1]|0,u=t[l+2]|0,m=t[l+3]|0,x=m+o*(255-m)/255|0;if(x<=0){t[l+0]=0,t[l+1]=0,t[l+2]=0,t[l+3]=0;continue}let g=i*o|0,d=r*o|0,v=a*o|0,b=f*m|0,D=c*m|0,w=u*m|0,M=255-m|0,_=b+g*M/255|0,P=D+d*M/255|0,T=w+v*M/255|0;t[l+0]=Math.min(255,Math.max(0,_*255/x|0)),t[l+1]=Math.min(255,Math.max(0,P*255/x|0)),t[l+2]=Math.min(255,Math.max(0,T*255/x|0)),t[l+3]=Math.min(255,Math.max(0,x))}}async export2DTextureToPNGBlob(t,e,i,r={}){if(!t)throw new Error("export2DTextureToPNGBlob: textureView is required");let a=Math.max(1,e|0),o=Math.max(1,i|0),s=r.layer??0,l=r.channel??0,f=this._resolveExportBackground(r.background);if(this.initBlitRender(),this.queue&&this.queue.onSubmittedWorkDone)try{await this.queue.onSubmittedWorkDone()}catch{}let u=this.device.createTexture({size:[a,o,1],format:"bgra8unorm",usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.COPY_SRC}),m=new Uint32Array([s>>>0,l>>>0,0,0]);this.queue.writeBuffer(this.blit2DUbo,0,m.buffer,m.byteOffset,m.byteLength);let x=this.device.createBindGroup({layout:this.bgl2D,entries:[{binding:0,resource:this.sampler},{binding:1,resource:t},{binding:2,resource:{buffer:this.blit2DUbo}}]}),g=this.device.createCommandEncoder(),d=g.beginRenderPass({colorAttachments:[{view:u.createView(),loadOp:"clear",storeOp:"store",clearValue:{r:0,g:0,b:0,a:0}}]});d.setPipeline(this.pipeline2D),d.setBindGroup(0,x),d.draw(6,1,0,0),d.end();let v=4,b=256,D=a*v,w=Math.ceil(D/b)*b,M=w*o,_=this.device.createBuffer({size:M,usage:GPUBufferUsage.COPY_DST|GPUBufferUsage.MAP_READ});g.copyTextureToBuffer({texture:u},{buffer:_,bytesPerRow:w,rowsPerImage:o},{width:a,height:o,depthOrArrayLayers:1}),this.queue.submit([g.finish()]),this.queue&&this.queue.onSubmittedWorkDone&&await this.queue.onSubmittedWorkDone(),await _.mapAsync(GPUMapMode.READ);let P=_.getMappedRange(),T=new Uint8Array(P),A=new Uint8ClampedArray(a*o*v),C=0;for(let q=0;q<o;q++){let y=q*w;for(let z=0;z<a;z++){let p=y+z*4;A[C++]=T[p+2],A[C++]=T[p+1],A[C++]=T[p+0],A[C++]=T[p+3]}}_.unmap(),_.destroy(),u.destroy(),this._applyExportBackground(A,f);let B=document.createElement("canvas");B.width=a,B.height=o;let R=B.getContext("2d");if(!R)throw new Error("export2DTextureToPNGBlob: unable to get 2D context");return R.putImageData(new ImageData(A,a,o),0,0),await new Promise((q,y)=>{B.toBlob(z=>{z?q(z):y(new Error("export2DTextureToPNGBlob: toBlob returned null"))},"image/png")})}async exportCurrent2DToPNGBlob(t,e,i={}){let r=this.getCurrentView();if(!r)throw new Error("exportCurrent2DToPNGBlob: no active 2D texture view");return this.export2DTextureToPNGBlob(r,t,e,i)}async export3DSliceToPNGBlob(t,e,i,r={}){if(!t)throw new Error("export3DSliceToPNGBlob: target is required");let a=Math.max(1,e|0),o=Math.max(1,i|0),{depth:s,slice:l=0,zNorm:f=null,channel:c=0,chunk:u=0}=r;if(!s||s<=0)throw new Error("export3DSliceToPNGBlob: depth must be provided and > 0");let m=this._resolveExportBackground(r.background);if(this.initBlitRender(),this.queue&&this.queue.onSubmittedWorkDone)try{await this.queue.onSubmittedWorkDone()}catch{}let x,g;if(t&&t.views&&Array.isArray(t.views)){let N=Math.max(0,Math.min(u|0,t.views.length-1));x=t.views[N],g=t.meta?.tile?.d??s}else x=t,g=s;if(!x||!g)throw new Error("export3DSliceToPNGBlob: need a 3D view and its depth");let d=f??(Math.min(Math.max(l,0),g-1)+.5)/g;d=Math.min(Math.max(d,0),1);let b=this.device.createTexture({size:[a,o,1],format:"bgra8unorm",usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.COPY_SRC}),D=new ArrayBuffer(16),w=new DataView(D);w.setFloat32(0,d,!0),w.setUint32(4,c>>>0,!0),w.setUint32(8,0,!0),w.setUint32(12,0,!0),this.queue.writeBuffer(this.blit3DUbo,0,D);let M=this.device.createBindGroup({layout:this.bgl3D,entries:[{binding:0,resource:this.sampler},{binding:1,resource:x},{binding:2,resource:{buffer:this.blit3DUbo}}]}),_=this.device.createCommandEncoder(),P=_.beginRenderPass({colorAttachments:[{view:b.createView(),loadOp:"clear",storeOp:"store",clearValue:{r:0,g:0,b:0,a:0}}]});P.setPipeline(this.pipeline3D),P.setBindGroup(0,M),P.draw(6,1,0,0),P.end();let T=4,A=256,C=a*T,B=Math.ceil(C/A)*A,R=B*o,S=this.device.createBuffer({size:R,usage:GPUBufferUsage.COPY_DST|GPUBufferUsage.MAP_READ});_.copyTextureToBuffer({texture:b},{buffer:S,bytesPerRow:B,rowsPerImage:o},{width:a,height:o,depthOrArrayLayers:1}),this.queue.submit([_.finish()]),this.queue&&this.queue.onSubmittedWorkDone&&await this.queue.onSubmittedWorkDone(),await S.mapAsync(GPUMapMode.READ);let q=S.getMappedRange(),y=new Uint8Array(q),z=new Uint8ClampedArray(a*o*T),p=0;for(let N=0;N<o;N++){let k=N*B;for(let I=0;I<a;I++){let F=k+I*4;z[p++]=y[F+2],z[p++]=y[F+1],z[p++]=y[F+0],z[p++]=y[F+3]}}S.unmap(),S.destroy(),b.destroy(),this._applyExportBackground(z,m);let h=document.createElement("canvas");h.width=a,h.height=o;let V=h.getContext("2d");if(!V)throw new Error("export3DSliceToPNGBlob: unable to get 2D context");return V.putImageData(new ImageData(z,a,o),0,0),await new Promise((N,k)=>{h.toBlob(I=>{I?N(I):k(new Error("export3DSliceToPNGBlob: toBlob returned null"))},"image/png")})}async _render3DSliceToRGBA8Pixels(t,e,i,r,a=0,o=null){if(!t)throw new Error("_render3DSliceToRGBA8Pixels: view3D is required");let s=Math.max(1,e|0),l=Math.max(1,i|0);this.initBlitRender();let f=Math.min(Math.max(Number(r)||0,0),1),u=this.device.createTexture({size:[s,l,1],format:"bgra8unorm",usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.COPY_SRC}),m=new ArrayBuffer(16),x=new DataView(m);x.setFloat32(0,f,!0),x.setUint32(4,a>>>0,!0),x.setUint32(8,0,!0),x.setUint32(12,0,!0),this.queue.writeBuffer(this.blit3DUbo,0,m);let g=this.device.createBindGroup({layout:this.bgl3D,entries:[{binding:0,resource:this.sampler},{binding:1,resource:t},{binding:2,resource:{buffer:this.blit3DUbo}}]}),d=this.device.createCommandEncoder(),v=d.beginRenderPass({colorAttachments:[{view:u.createView(),loadOp:"clear",storeOp:"store",clearValue:{r:0,g:0,b:0,a:0}}]});v.setPipeline(this.pipeline3D),v.setBindGroup(0,g),v.draw(6,1,0,0),v.end();let b=4,D=256,w=s*b,M=Math.ceil(w/D)*D,_=M*l,P=this.device.createBuffer({size:_,usage:GPUBufferUsage.COPY_DST|GPUBufferUsage.MAP_READ});d.copyTextureToBuffer({texture:u},{buffer:P,bytesPerRow:M,rowsPerImage:l},{width:s,height:l,depthOrArrayLayers:1}),this.queue.submit([d.finish()]),this.queue&&this.queue.onSubmittedWorkDone&&await this.queue.onSubmittedWorkDone(),await P.mapAsync(GPUMapMode.READ);let T=P.getMappedRange(),A=new Uint8Array(T),C=new Uint8ClampedArray(s*l*b),B=0;for(let R=0;R<l;R++){let S=R*M;for(let q=0;q<s;q++){let y=S+q*4;C[B++]=A[y+2],C[B++]=A[y+1],C[B++]=A[y+0],C[B++]=A[y+3]}}return P.unmap(),P.destroy(),u.destroy(),o&&this._applyExportBackground(C,o),C}async export3DTilesetToPNGBlob(t,e,i,r={}){if(!t)throw new Error("export3DTilesetToPNGBlob: target is required");let a=Math.max(1,e|0),o=Math.max(1,(i??e)|0),{depth:s,channel:l=0,chunk:f=0,tilesAcross:c=16,tilesDown:u=null,startSlice:m=0,sliceCount:x=null}=r,g=this._resolveExportBackground(r.background);if(this.initBlitRender(),this.queue&&this.queue.onSubmittedWorkDone)try{await this.queue.onSubmittedWorkDone()}catch{}let d,v;if(t&&t.views&&Array.isArray(t.views)){let S=Math.max(0,Math.min(f|0,t.views.length-1));d=t.views[S],v=t.meta?.tile?.d??s}else d=t,v=s;if(!d)throw new Error("export3DTilesetToPNGBlob: missing 3D view");if(!v||v<=0)throw new Error("export3DTilesetToPNGBlob: depth must be provided and > 0");let b=Math.max(1,c|0),D=u!=null?Math.max(1,u|0):Math.ceil(v/b),w=Math.min(Math.max(m|0,0),v-1),M=x!=null?Math.max(0,x|0):v-w,_=a*b,P=o*D,T=new Uint8ClampedArray(_*P*4),A=Math.min(v,w+M);for(let S=w;S<A;S++){let q=S-w,y=q%b,z=q/b|0;if(z>=D)break;let p=(S+.5)/v,h=await this._render3DSliceToRGBA8Pixels(d,a,o,p,l,g),V=y*a,L=z*o;for(let N=0;N<o;N++){let k=N*a*4,I=((L+N)*_+V)*4;T.set(h.subarray(k,k+a*4),I)}}let C=document.createElement("canvas");C.width=_,C.height=P;let B=C.getContext("2d");if(!B)throw new Error("export3DTilesetToPNGBlob: unable to get 2D context");return B.putImageData(new ImageData(T,_,P),0,0),await new Promise((S,q)=>{C.toBlob(y=>{y?S(y):q(new Error("export3DTilesetToPNGBlob: toBlob returned null"))},"image/png")})}},te=class{constructor(t=Date.now()){t<1e7&&(t*=1e7),this.seedN=t,this.seedK=t,this.perm=new Uint8Array(512),this.seed(t)}seed(t){let e=this.xorshift(t);for(let i=0;i<256;i++)this.perm[i]=i;for(let i=255;i>0;i--){let r=Math.floor(e()*(i+1));[this.perm[i],this.perm[r]]=[this.perm[r],this.perm[i]]}for(let i=0;i<256;i++)this.perm[i+256]=this.perm[i]}setSeed(t){this.seedN=t,this.seed(t),this.resetSeed()}random(t,e,i){let r;return typeof i=="number"?r=this.perm[(t&255)+this.perm[(e&255)+this.perm[i&255]]]&255:r=this.perm[(t&255)+this.perm[e&255]]&255,this.perm[r]/255*2-1}seededRandom(){this.seedK+=Math.E;let t=1e9*Math.sin(this.seedK);return t-Math.floor(t)}resetSeed(){this.seedK=this.seedN}xorshift(t){let e=t;return function(){return e^=e<<13,e^=e>>17,e^=e<<5,(e<0?1+~e:e)/4294967295}}dot(t,e=0,i=0,r=0){return t[0]*e+t[1]*i+t[2]*r}};document.body.insertAdjacentHTML("afterbegin",le);var Be=6,Pe={computeCellular:"CellularPattern",computeWorley:"WorleyPattern",computeAntiCellular:"AntiCellularPattern",computeAntiWorley:"AntiWorleyPattern",computeWhiteNoise:"White Noise",computeBlueNoise:"Blue Noise"},Ce={computeRidge:{clamp:{freq:[.25,8],gain:[.2,.8],octaves:[1,12]}},computeAntiRidge:{clamp:{freq:[.25,8],gain:[.2,.8],octaves:[1,12]}},computeRidgedMultifractal:{clamp:{freq:[.25,8],gain:[.2,.9],octaves:[2,14]}},computeRidgedMultifractal2:{clamp:{freq:[.25,8],gain:[.2,.9],octaves:[2,14]}},computeRidgedMultifractal3:{clamp:{freq:[.25,8],gain:[.2,.9],octaves:[2,14]}},computeRidgedMultifractal4:{clamp:{freq:[.25,8],gain:[.2,.9],octaves:[2,14]}},computeFBM:{clamp:{gain:[.2,.8],octaves:[2,10]}},computeFBM2:{clamp:{gain:[.2,.8],octaves:[2,10]}},computeFBM3:{clamp:{gain:[.2,.8],octaves:[2,10]}},computeVoronoiBM1:{clamp:{threshold:[0,1],edgeK:[0,64]}},computeVoronoiBM2:{clamp:{threshold:[0,1],edgeK:[0,64]}},computeVoronoiBM3:{clamp:{threshold:[0,1],edgeK:[0,64]}},computeCellular:{clamp:{threshold:[0,1]}},computeWorley:{clamp:{threshold:[0,1]}},computeAntiCellular:{clamp:{threshold:[0,1]}},computeAntiWorley:{clamp:{threshold:[0,1]}},computeSimplexFBM:{force:{turbulence:1},clamp:{warpAmp:[.1,2],freq:[.25,6]}},computeCurl2D:{force:{turbulence:1},clamp:{warpAmp:[.1,2],freq:[.25,6]}},computeCurlFBM2D:{force:{turbulence:1},clamp:{warpAmp:[.1,3]}},computeDomainWarpFBM1:{force:{turbulence:1},clamp:{warpAmp:[.1,3]}},computeDomainWarpFBM2:{force:{turbulence:1},clamp:{warpAmp:[.1,3]}},computeGaborAnisotropic:{clamp:{gaborRadius:[.5,6]}},computeFoamNoise:{force:{turbulence:1},clamp:{gain:[.5,.95]}}},E=128,pe="toroidalDemo",Y=new Map,G=[],X=Object.create(null);function Z(n){let t=String(n||""),e=Pe[t];if(e)return e;let i=t;return i.startsWith("compute")&&(i=i.slice(7)),i||t}function Ae(n,t){let e=Object.create(null),i=Array.isArray(n)?n:[],r=Math.max(0,t|0),a=Math.max(0,i.length-r);for(let o=0;o<a;o++)e[o]=Z(i[o]);return e}function Te(){return Object.keys(X).map(n=>Number(n)).filter(n=>Number.isInteger(n)&&n>=0).sort((n,t)=>n-t)}function qe(){let n=[];for(let t=0;t<G.length;t++){let e=G[t];typeof e!="string"||!e||e!=="clearTexture"&&n.push(t)}return n}function Ne(n){let t=G[n];return t&&Ce[String(t)]||null}function O(n,t,e,i){if(!Object.prototype.hasOwnProperty.call(n,t))return;let r=Number(n[t]);if(!Number.isFinite(r))return;let a=Number(e),o=Number(i);n[t]=Math.min(Math.max(r,a),o)}function ce(n,t){let e={...t},i=Ne(n);if(i&&i.clamp){let a=i.clamp;a.freq&&O(e,"freq",a.freq[0],a.freq[1]),a.gain&&O(e,"gain",a.gain[0],a.gain[1]),a.octaves&&O(e,"octaves",a.octaves[0],a.octaves[1]),a.threshold&&O(e,"threshold",a.threshold[0],a.threshold[1]),a.warpAmp&&O(e,"warpAmp",a.warpAmp[0],a.warpAmp[1]),a.gaborRadius&&O(e,"gaborRadius",a.gaborRadius[0],a.gaborRadius[1]),a.edgeK&&O(e,"edgeK",a.edgeK[0],a.edgeK[1])}if(i&&i.force)for(let[a,o]of Object.entries(i.force))e[a]=o;let r=Y.get(n);if(r)for(let[a,o]of Object.entries(r))typeof o=="number"&&Number.isFinite(o)&&(e[a]=o);return e}function oe(){let n=(a,o)=>{let s=document.getElementById(a);if(!s)return o;let l=Number(s.value);return Number.isFinite(l)?l:o},t=(a,o)=>{let s=n(a,o);return Number.isFinite(s)?Math.max(0,Math.floor(s)):o},e=Math.max(1,Math.floor(n("noise-seed",1234567890))),i=document.getElementById("noise-turbulence"),r=i&&i.checked?1:0;return{seed:e,zoom:n("noise-zoom",1),freq:n("noise-freq",1),octaves:Math.max(1,Math.floor(n("noise-octaves",8))),lacunarity:n("noise-lacunarity",2),gain:n("noise-gain",.5),xShift:n("noise-xShift",0),yShift:n("noise-yShift",0),zShift:n("noise-zShift",0),turbulence:r,seedAngle:n("noise-seedAngle",0),exp1:n("noise-exp1",1),exp2:n("noise-exp2",0),threshold:n("noise-threshold",.1),rippleFreq:n("noise-rippleFreq",10),time:n("noise-time",0),warpAmp:n("noise-warpAmp",.5),gaborRadius:n("noise-gaborRadius",4),terraceStep:n("noise-terraceStep",8),toroidal:0,voroMode:t("noise-voroMode",0),edgeK:n("noise-edgeK",0)}}function Ee(){let n=document.querySelectorAll('input[type="checkbox"][name="noise-type"]'),t=[];return n.forEach(e=>{if(e.checked){let i=Number(e.dataset.bit);Number.isInteger(i)&&t.push(i)}}),t}function me(){let n=document.getElementById("z-slice"),t=document.getElementById("z-slice-num"),e=0;return n?e=Number(n.value):t&&(e=Number(t.value)),Number.isFinite(e)||(e=0),e=Math.min(Math.max(Math.round(e),0),E-1),n&&String(n.value)!==String(e)&&(n.value=String(e)),t&&String(t.value)!==String(e)&&(t.value=String(e)),e}function $(n,t=null,e=null){n.style.display="block",n.style.margin="0",n.style.padding="0",n.style.border="0",n.style.outline="0",n.style.background="transparent",n.style.width=t!=null?`${t}px`:"100%",n.style.height=e!=null?`${e}px`:"100%",n.style.imageRendering="crisp-edges",n.style.imageRendering="pixelated"}function ne(n,t,e,i,r=null,a=null){let o=Math.max(1,e|0),s=Math.max(1,i|0);$(t,r,a);let l=!1;return(t.width!==o||t.height!==s)&&(t.width=o,t.height=s,l=!0),n&&typeof n.configureCanvas=="function"&&l&&n.configureCanvas(t),l}function Re(n,t,e,i){let r=Math.max(1,i|0),a=Math.max(1,Math.round(Math.sqrt(r))),o=Math.ceil(r/a);n.style.display="grid",n.style.gridTemplateColumns=`repeat(${a}, ${t}px)`,n.style.gridAutoRows=`${e}px`,n.style.gap="0px",n.style.padding="0",n.style.margin="0",n.style.border="0",n.style.lineHeight="0",n.style.fontSize="0",n.style.alignItems="start",n.style.justifyItems="start",n.style.placeItems="start",n.style.overflow="hidden"}function ke(){let n=document.getElementById("noise-canvas"),t=document.getElementById("view-stack");if(!n&&t&&(n=document.createElement("canvas"),n.id="noise-canvas",n.width=800,n.height=800,t.appendChild(n)),!n)throw new Error("Missing main preview canvas (#noise-canvas)");$(n);let e=document.getElementById("mosaic");if(!e)throw new Error("Missing #mosaic container");let i=[],r=e.querySelectorAll("canvas");if(r.length)r.forEach(a=>{$(a),i.push(a)});else for(let a=0;a<9;a++){let o=document.createElement("canvas");o.width=E,o.height=E,$(o),e.appendChild(o),i.push(o)}return Re(e,E,E,i.length||9),{mainCanvas:n,mosaicCanvases:i}}function Ve(n){return n.length?n.map(e=>X[e]||String(e)).join(", "):X[0]||"Perlin"}function de(n){let t=document.getElementById(n);if(!t)throw new Error(`Missing #${n}`);return t}function Le(){let n=de("noise-type-list");n.innerHTML="";let t=Te();for(let e of t){let i=document.createElement("label"),r=document.createElement("input");r.type="checkbox",r.name="noise-type",r.dataset.bit=String(e),e===0&&(r.checked=!0),i.appendChild(r),i.appendChild(document.createTextNode(" "+(X[e]||String(e)))),n.appendChild(i)}}function Ie(n){return(Array.isArray(n)?n:[]).filter(e=>typeof e=="string"&&/4D/.test(e)&&e!=="clearTexture").slice()}function Ge(n){let t=de("toroidal-type-list");t.innerHTML="";let e=Ie(n),i=new Set(["computePerlin4D","computeWorley4D"]),r=!1;for(let a of e){let o=document.createElement("label"),s=document.createElement("input");s.type="checkbox",s.name="toroidal-type",s.dataset.entry=a;let l=G.indexOf(a);Number.isInteger(l)&&l>=0&&(s.dataset.bit=String(l)),i.has(a)&&(s.checked=!0,r=!0),o.appendChild(s),o.appendChild(document.createTextNode(" "+Z(a))),t.appendChild(o)}if(!r&&e.length){let a=t.querySelector('input[type="checkbox"][name="toroidal-type"]');a&&(a.checked=!0)}}function ae(){let n=document.querySelectorAll('input[type="checkbox"][name="toroidal-type"]'),t=[];if(n.forEach(e=>{if(!e.checked)return;let i=String(e.dataset.entry||"");if(!i)return;let r=Number(e.dataset.bit);Number.isInteger(r)||(r=G.indexOf(i)),Number.isInteger(r)||(r=-1),t.push({bit:r,entry:i})}),!t.length){let e=["computePerlin4D","computeWorley4D"];for(let i of e){if(!G.includes(i))continue;let r=G.indexOf(i);t.push({bit:r,entry:i})}}return t}function ge(n){let t=document.getElementById("mosaic-caption");if(!t)return;let e=Array.isArray(n)?n:[],i=e.length?e.map(r=>Z(r)).join(" + "):"None";t.textContent=`A single toroidal Z slice from a 4D volume. Modes: ${i}. Repeated in X and Y. Use the Z slice control to see different slices.`}function Fe(){let n=document.getElementById("override-mode");if(!n)return;n.innerHTML="";let t=qe();for(let e of t){let i=G[e],r=document.createElement("option");r.value=String(e),r.textContent=`${e}: ${Z(i)}`,n.appendChild(r)}t.length&&(n.value=String(t[0]))}function ie(n){let t=Y.get(n)||{},e=(r,a)=>{let o=document.getElementById(r);if(!o)return;let s=t[a];o.value=typeof s=="number"&&Number.isFinite(s)?String(s):""},i=(r,a)=>{let o=document.getElementById(r);if(!o)return;let s=t[a];o.value=typeof s=="number"&&Number.isFinite(s)?String(s):""};e("ov-zoom","zoom"),e("ov-freq","freq"),e("ov-lacunarity","lacunarity"),e("ov-gain","gain"),e("ov-octaves","octaves"),i("ov-turbulence","turbulence"),e("ov-seedAngle","seedAngle"),e("ov-exp1","exp1"),e("ov-exp2","exp2"),e("ov-rippleFreq","rippleFreq"),e("ov-time","time"),e("ov-warp","warpAmp"),e("ov-threshold","threshold"),i("ov-voroMode","voroMode"),e("ov-edgeK","edgeK"),e("ov-gabor","gaborRadius"),e("ov-terraceStep","terraceStep"),e("ov-xShift","xShift"),e("ov-yShift","yShift"),e("ov-zShift","zShift")}function K(){let n=document.getElementById("override-mode");if(!n)return;let t=Number(n.value);if(!Number.isInteger(t))return;let e=C=>{let B=document.getElementById(C);if(!B)return null;let R=String(B.value).trim();if(!R)return null;let S=Number(R);return Number.isFinite(S)?S:null},i=C=>{let B=document.getElementById(C);if(!B)return null;let R=String(B.value).trim();if(!R)return null;let S=Number(R);return Number.isFinite(S)?S:null},r={},a=e("ov-zoom"),o=e("ov-freq"),s=e("ov-lacunarity"),l=e("ov-gain"),f=e("ov-octaves"),c=i("ov-turbulence"),u=e("ov-seedAngle"),m=e("ov-exp1"),x=e("ov-exp2"),g=e("ov-rippleFreq"),d=e("ov-time"),v=e("ov-warp"),b=e("ov-threshold"),D=i("ov-voroMode"),w=e("ov-edgeK"),M=e("ov-gabor"),_=e("ov-terraceStep"),P=e("ov-xShift"),T=e("ov-yShift"),A=e("ov-zShift");a!==null&&(r.zoom=a),o!==null&&(r.freq=o),s!==null&&(r.lacunarity=s),l!==null&&(r.gain=l),f!==null&&(r.octaves=f),c!==null&&(r.turbulence=Math.max(0,Math.floor(c))),u!==null&&(r.seedAngle=u),m!==null&&(r.exp1=m),x!==null&&(r.exp2=x),g!==null&&(r.rippleFreq=g),d!==null&&(r.time=d),v!==null&&(r.warpAmp=v),b!==null&&(r.threshold=b),D!==null&&(r.voroMode=Math.max(0,Math.floor(D))),w!==null&&(r.edgeK=w),M!==null&&(r.gaborRadius=M),_!==null&&(r.terraceStep=_),P!==null&&(r.xShift=P),T!==null&&(r.yShift=T),A!==null&&(r.zShift=A),Object.keys(r).length?Y.set(t,r):Y.delete(t)}function ue(n){return typeof n=="string"&&/4d/i.test(n)}async function re(n,t){let e=Number(document.getElementById("res-width")?.value)||800,i=Number(document.getElementById("res-height")?.value)||800;ne(n,t,e,i);let r=document.getElementById("preview-meta"),a=document.getElementById("preview-stats"),o=oe();n.buildPermTable(o.seed|0);let s=Ee(),l=s.length?s:[0],f={getGradient:0,outputChannel:1,baseRadius:0,heightScale:1,useCustomPos:0},c=performance.now();await n.computeToTexture(e,i,o,{...f,noiseChoices:["clearTexture"],frameFullWidth:e,frameFullHeight:i});for(let d of l){let v=G[d],b=ce(d,o);b.toroidal=ue(v)?1:0,await n.computeToTexture(e,i,b,{...f,noiseChoices:[d],frameFullWidth:e,frameFullHeight:i})}let u=performance.now(),m=n.getCurrentView(),x=performance.now();m&&n.renderTextureToCanvas(m,t,{layer:0,channel:0,preserveCanvasSize:!0,clear:!0});let g=performance.now();if(r){let v=l.some(b=>ue(G[b]))?" \xB7 toroidal(4D)":"";r.textContent=`Height field preview \xB7 ${e}\xD7${i} \xB7 modes: ${Ve(l)}${v}`}if(a){let d=(u-c).toFixed(1),v=(g-x).toFixed(1);a.textContent=`GPU compute ${d} ms \xB7 blit ${v} ms`}return{resW:e,resH:i,noiseBits:l}}async function j(n,t,e){let i=oe();n.buildPermTable(i.seed|0);let r={...i,toroidal:1},a=ae();ge(a.map(f=>f.entry));let o=performance.now(),s=await n.computeToTexture3D(E,E,E,r,{noiseChoices:["clearTexture"],outputChannel:1,id:pe});for(let f of a){let c=f.bit,u=f.entry,m=Number.isInteger(c)&&c>=0?ce(c,r):{...r};m.toroidal=1,s=await n.computeToTexture3D(E,E,E,m,{noiseChoices:[u],outputChannel:1,id:pe})}let l=performance.now();e.lastToroidalVolumeView=s,e.lastToroidalComputeMs=l-o,ve(n,s,new Array(9).fill(0).map((f,c)=>t[c]))}function ve(n,t,e){if(!t)return;let i=E,a=(me()+.5)/i,o=Array.isArray(e)?e:[],s=o.length||9;for(let l=0;l<s;l++){let f=o[l];f&&(ne(n,f,E,E),n.renderTexture3DSliceToCanvas(t,f,{depth:i,zNorm:a,channel:0,chunk:0,preserveCanvasSize:!0,clear:!0}))}}async function We(){let n=document.getElementById("preview-stats");if(!navigator.gpu){console.error("WebGPU not available in this browser."),n&&(n.textContent="WebGPU not available in this browser.");return}let t=await navigator.gpu.requestAdapter();if(!t){console.error("Failed to get GPU adapter."),n&&(n.textContent="Failed to get GPU adapter.");return}let e=await t.requestDevice(),i=new H(e,e.queue);G=Array.isArray(i.entryPoints)?i.entryPoints.slice():[],X=Ae(G,Be),Le(),Ge(G),Fe();let{mainCanvas:r,mosaicCanvases:a}=ke();i.configureCanvas(r),a.forEach(p=>i.configureCanvas(p));let o=document.getElementById("override-mode");if(o){let p=Number(o.value);Number.isInteger(p)&&ie(p)}let s={lastToroidalVolumeView:null,lastToroidalComputeMs:0},l=!1,f=!1,c=()=>{l||(l=!0,requestAnimationFrame(()=>{l=!1,re(i,r).catch(p=>{console.error(p),n&&(n.textContent=String(p))})}))},u=()=>{f||(f=!0,requestAnimationFrame(()=>{f=!1,j(i,a,s).catch(p=>{console.error(p),n&&(n.textContent=String(p))})}))},m=()=>{c(),u()};["ov-zoom","ov-freq","ov-lacunarity","ov-gain","ov-octaves","ov-turbulence","ov-seedAngle","ov-exp1","ov-exp2","ov-rippleFreq","ov-time","ov-warp","ov-threshold","ov-voroMode","ov-edgeK","ov-gabor","ov-terraceStep","ov-xShift","ov-yShift","ov-zShift"].forEach(p=>{let h=document.getElementById(p);h&&h.addEventListener("change",()=>{K(),c(),u()})}),o&&o.addEventListener("change",()=>{let p=Number(o.value);Number.isInteger(p)&&ie(p)});let g=document.getElementById("ov-clear");g&&g.addEventListener("click",()=>{let p=document.getElementById("override-mode");if(!p)return;let h=Number(p.value);Number.isInteger(h)&&(Y.delete(h),ie(h),c(),u())});let d=document.getElementById("render-btn");d&&d.addEventListener("click",()=>{m()});let v=document.getElementById("apply-res");v&&v.addEventListener("click",()=>{m()}),["noise-seed","noise-zoom","noise-freq","noise-octaves","noise-lacunarity","noise-gain","noise-xShift","noise-yShift","noise-zShift","noise-voroMode","noise-threshold","noise-edgeK","noise-seedAngle","noise-turbulence","noise-time","noise-warpAmp","noise-gaborRadius","noise-terraceStep","noise-exp1","noise-exp2","noise-rippleFreq"].forEach(p=>{let h=document.getElementById(p);h&&(h.addEventListener("input",()=>{c(),u()}),h.addEventListener("change",()=>{c(),u()}))});let D=document.getElementById("noise-type-list");D&&D.addEventListener("change",p=>{let h=p.target;!h||h.name!=="noise-type"||c()});let w=document.getElementById("toroidal-type-list");w&&w.addEventListener("change",p=>{let h=p.target;!h||h.name!=="toroidal-type"||u()});let M=document.getElementById("z-slice"),_=document.getElementById("z-slice-num"),P=()=>{s.lastToroidalVolumeView&&ve(i,s.lastToroidalVolumeView,a)};M&&M.addEventListener("input",()=>{let p=Number(M.value);_&&(_.value=String(p)),P()}),_&&_.addEventListener("change",()=>{let p=Number(_.value);Number.isFinite(p)||(p=0),p=Math.min(Math.max(Math.round(p),0),E-1),_.value=String(p),M&&(M.value=String(p)),P()});let T=document.getElementById("download-main");T&&T.addEventListener("click",async()=>{try{K();let p=y(i),h=Number(document.getElementById("res-width")?.value)||800,V=Number(document.getElementById("res-height")?.value)||800;ne(i,r,h,V),await re(i,r);let L=await i.exportCurrent2DToPNGBlob(h,V,{layer:0,channel:0,background:p}),N=URL.createObjectURL(L),k=document.createElement("a");k.href=N,k.download="noise-main.png",document.body.appendChild(k),k.click(),k.remove(),URL.revokeObjectURL(N)}catch(p){console.error("download-main failed:",p),n&&(n.textContent="Export main PNG failed: "+p)}});let A=document.getElementById("download-tile");A&&A.addEventListener("click",async()=>{try{K();let p=y(i);if(await j(i,a,s),!s.lastToroidalVolumeView){console.warn("No toroidal volume available for export");return}let h=(a&&a.length?a[0]:null)||document.getElementById("tile-canvas");if(!h){console.warn("No tile canvas found for export");return}let V=h.width||E,L=h.height||E,N=E,I=(me()+.5)/N,F=await i.export3DSliceToPNGBlob(s.lastToroidalVolumeView,V,L,{depth:N,zNorm:I,channel:0,chunk:0,background:p}),U=URL.createObjectURL(F),W=document.createElement("a");W.href=U,W.download="noise-tile.png",document.body.appendChild(W),W.click(),W.remove(),URL.revokeObjectURL(U)}catch(p){console.error("download-tile failed:",p),n&&(n.textContent="Export tile PNG failed: "+p)}});function C(p,h){let V=URL.createObjectURL(p),L=document.createElement("a");L.href=V,L.download=h,document.body.appendChild(L),L.click(),L.remove(),URL.revokeObjectURL(V)}function B(p){return String(p||"").trim().replace(/\s+/g,"_").replace(/[^a-zA-Z0-9._-]+/g,"").slice(0,120)}async function R(p,h){if(!h||!h.lastToroidalVolumeView){console.warn("No toroidal volume available for tileset export");return}K();let V=y(p);await j(p,a,h);let L=oe(),N=ae().map(be=>be.entry),k=16,I=E,F=E,U=E,W=Math.ceil(U/k),xe=await p.export3DTilesetToPNGBlob(h.lastToroidalVolumeView,I,F,{depth:U,channel:0,chunk:0,tilesAcross:k,tilesDown:W,startSlice:0,sliceCount:U,background:V}),he=B(N.map(Z).join("+"))||"tileset",ye=B(L.seed),ze=`noise-tileset_${he}_seed${ye}_${I}x${F}_z${U}_${k}x${W}.png`;C(xe,ze)}let S=document.getElementById("download-tileset");S&&S.addEventListener("click",async()=>{try{await R(i,s)}catch(p){console.error("download-tileset failed:",p),n&&(n.textContent="Export tileset failed: "+p)}});function q(){let p=document.querySelector('input[type="radio"][name="export-bg"]:checked'),h=String(p?.value||"transparent");return h==="black"||h==="white"||h==="transparent"?h:"transparent"}function y(p){let h=q();return p&&typeof p.setExportBackground=="function"&&p.setExportBackground(h),h}function z(p){y(p),document.querySelectorAll('input[type="radio"][name="export-bg"]').forEach(V=>{V.addEventListener("change",()=>{y(p)})})}z(i),ge(ae().map(p=>p.entry)),await re(i,r),await j(i,a,s)}document.addEventListener("DOMContentLoaded",()=>{We().catch(n=>console.error(n))});})();
