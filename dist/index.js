...
`;var V=`const PI : f32 = 3.141592653589793;\r
...
`;var I=4096,T=2048,$=8,B=class{constructor(t,e){this.device=t,this.queue=e,this.entryPoints=["computePerlin","computeBillow","computeAntiBillow","computeRidge","computeAntiRidge","computeRidgedMultifractal","computeRidgedMultifractal2","computeRidgedMultifractal3","computeRidgedMultifractal4","computeAntiRidgedMultifractal","computeAntiRidgedMultifractal2","computeAntiRidgedMultifractal3","computeAntiRidgedMultifractal4","computeFBM","computeFBM2","computeFBM3","computeCellularBM1","computeCellularBM2","computeCellularBM3","computeVoronoiBM1","computeVoronoiBM2","computeVoronoiBM3","computeCellular","computeWorley","computeAntiCellular","computeAntiWorley","computeLanczosBillow","computeLanczosAntiBillow","computeVoronoiTileNoise","computeVoronoiCircleNoise","computeVoronoiCircle2","computeVoronoiFlatShade","computeVoronoiRipple3D","computeVoronoiRipple3D2","computeVoronoiCircularRipple","computeFVoronoiRipple3D","computeFVoronoiCircularRipple","computeRippleNoise","computeFractalRipples","computeHexWorms","computePerlinWorms","computeWhiteNoise","computeBlueNoise","computeSimplex","computeCurl2D","computeCurlFBM2D","computeDomainWarpFBM1","computeDomainWarpFBM2","computeGaborAnisotropic","computeTerraceNoise","computeFoamNoise","computeTurbulence","computePerlin4D","computeWorley4D","computeAntiWorley4D","computeGauss5x5","computeNormal","computeNormal8","computeSphereNormal","computeNormalVolume","clearTexture"],this.shaderModule=t.createShaderModule({code:V}),this.bindGroupLayout=t.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}},{binding:1,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}},{binding:2,visibility:GPUShaderStage.COMPUTE,buffer:{type:"read-only-storage"}},{binding:3,visibility:GPUShaderStage.COMPUTE,texture:{sampleType:"float",viewDimension:"2d-array"}},{binding:4,visibility:GPUShaderStage.COMPUTE,storageTexture:{access:"write-only",format:"rgba16float",viewDimension:"2d-array"}},{binding:5,visibility:GPUShaderStage.COMPUTE,buffer:{type:"read-only-storage"}},{binding:6,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}},{binding:7,visibility:GPUShaderStage.COMPUTE,texture:{sampleType:"float",viewDimension:"3d"}},{binding:8,visibility:GPUShaderStage.COMPUTE,storageTexture:{access:"write-only",format:"rgba16float",viewDimension:"3d"}}]}),this.pipelineLayout=t.createPipelineLayout({bindGroupLayouts:[this.bindGroupLayout]}),this.pipelines=new Map,this._texPairs=new Map,this._tid=null,this._tag=new WeakMap,this._volumeCache=new Map,this.viewA=null,this.viewB=null,this.width=0,this.height=0,this.layers=1,this.isA=!0,this._initBuffers(),this._ensureDummies(),this._ctxMap=new WeakMap}_initBuffers(){this.optionsBuffer?.destroy(),this.paramsBuffer?.destroy(),this.permBuffer?.destroy(),this.nullPosBuffer?.destroy(),this.optionsBuffer=this.device.createBuffer({size:32,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),this.paramsBuffer=this.device.createBuffer({size:88,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),this.permBuffer=this.device.createBuffer({size:512*4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}),this.nullPosBuffer=this.device.createBuffer({size:64,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}),this.queue.writeBuffer(this.optionsBuffer,0,new ArrayBuffer(32)),this.queue.writeBuffer(this.paramsBuffer,0,new ArrayBuffer(88)),this.queue.writeBuffer(this.permBuffer,0,new Uint32Array(512))}_ensureDummies(){this._dummy2D_sampleTex||(this._dummy2D_sampleTex=this.device.createTexture({size:[1,1,1],format:"rgba16float",usage:GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_SRC}),this._dummy2D_sampleView=this._dummy2D_sampleTex.createView({dimension:"2d-array",arrayLayerCount:1})),this._dummy2D_writeTex||(this._dummy2D_writeTex=this.device.createTexture({size:[1,1,1],format:"rgba16float",usage:GPUTextureUsage.STORAGE_BINDING|GPUTextureUsage.COPY_DST}),this._dummy2D_writeView=this._dummy2D_writeTex.createView({dimension:"2d-array",arrayLayerCount:1})),this._dummy3D_sampleTex||(this._dummy3D_sampleTex=this.device.createTexture({size:{width:1,height:1,depthOrArrayLayers:1},dimension:"3d",format:"rgba16float",usage:GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_SRC}),this._dummy3D_sampleView=this._dummy3D_sampleTex.createView({dimension:"3d"})),this._dummy3D_writeTex||(this._dummy3D_writeTex=this.device.createTexture({size:{width:1,height:1,depthOrArrayLayers:1},dimension:"3d",format:"rgba16float",usage:GPUTextureUsage.STORAGE_BINDING|GPUTextureUsage.COPY_DST}),this._dummy3D_writeView=this._dummy3D_writeTex.createView({dimension:"3d"}))}resize(t){this.maxConfigs=t,this._initBuffers()}setPermTable(t){this.queue.writeBuffer(this.permBuffer,0,t)}setPosBuffer(t){this.posBuffer=t}setInputTextureView(t){try{if(((t?.texture?.usage??0)&GPUTextureUsage.TEXTURE_BINDING)===0){console.warn("setInputTextureView: provided texture view not created with TEXTURE_BINDING; ignoring.");return}}catch{}if(this.inputTextureView=t,this._tid!==null){let e=this._texPairs.get(this._tid);e&&(e.bindGroupDirty=!0)}}setOutputTextureView(t){try{if(((t?.texture?.usage??0)&GPUTextureUsage.STORAGE_BINDING)===0){console.warn("setOutputTextureView: provided texture view not created with STORAGE_BINDING; ignoring.");return}}catch{}if(this.outputTextureView=t,this._tid!==null){let e=this._texPairs.get(this._tid);e&&(e.bindGroupDirty=!0)}}buildPermTable(t=Date.now()){let i=new R(t).perm,a=new Uint32Array(512);for(let r=0;r<512;r++)a[r]=i[r];this.setPermTable(a)}setOptions(t={}){Array.isArray(t.noiseChoices)?this.noiseChoices=t.noiseChoices:this.noiseChoices||(this.noiseChoices=[0]);let{getGradient:e=0,outputChannel:i=1,baseRadius:a=0,heightScale:r=1,useCustomPos:o=0,ioFlags:s=0}=t;this.useCustomPos=o>>>0;let l=new ArrayBuffer(32),f=new DataView(l);f.setUint32(0,e,!0),f.setUint32(4,this.useCustomPos,!0),f.setUint32(8,i,!0),f.setUint32(12,s>>>0,!0),f.setFloat32(16,a,!0),f.setFloat32(20,r,!0),f.setFloat32(24,0,!0),f.setFloat32(28,0,!0),this.queue.writeBuffer(this.optionsBuffer,0,l);for(let u of this._texPairs.values())u.bindGroupDirty=!0}setNoiseParams(t={}){let{seed:e=Date.now()|0,zoom:i=1,freq:a=1,octaves:r=8,lacunarity:o=2,gain:s=.5,xShift:l=0,yShift:f=0,zShift:u=0,turbulence:p=0,seedAngle:c=0,exp1:y=1,exp2:m=0,threshold:d=.1,rippleFreq:g=10,time:z=0,warpAmp:_=.5,gaborRadius:w=4,terraceStep:x=8,toroidal:b=0,voroMode:Y=0,edgeK:X=0}=t,fe=Math.max(i,1e-6),pe=Math.max(a,1e-6),O=(b?1:0)>>>0,k=new ArrayBuffer(88),h=new DataView(k),v=0;h.setUint32(v+0,e>>>0,!0),h.setFloat32(v+4,i,!0),h.setFloat32(v+8,a,!0),h.setUint32(v+12,r>>>0,!0),h.setFloat32(v+16,o,!0),h.setFloat32(v+20,s,!0),h.setFloat32(v+24,l,!0),h.setFloat32(v+28,f,!0),h.setFloat32(v+32,u,!0),h.setUint32(v+36,p?1:0,!0),h.setFloat32(v+40,c,!0),h.setFloat32(v+44,y,!0),h.setFloat32(v+48,m,!0),h.setFloat32(v+52,d,!0),h.setFloat32(v+56,g,!0),h.setFloat32(v+60,z,!0),h.setFloat32(v+64,_,!0),h.setFloat32(v+68,w,!0),h.setFloat32(v+72,x,!0),h.setUint32(v+76,O>>>0,!0),h.setUint32(v+80,Y>>>0,!0),h.setFloat32(v+84,X,!0),this.queue.writeBuffer(this.paramsBuffer,0,k);for(let L of this._texPairs.values())L.bindGroupDirty=!0;for(let[L,P]of this._volumeCache)!P||!Array.isArray(P.chunks)||(P._bindGroupsDirty=!0)}_compute2DTiling(t,e){let i=Math.min(t,I),a=Math.min(e,I),r=Math.ceil(t/i),o=Math.ceil(e/a),s=r*o;return{tileW:i,tileH:a,tilesX:r,tilesY:o,layers:s}}_create2DPair(t,e){let i=this._compute2DTiling(t,e),a=GPUTextureUsage.STORAGE_BINDING|GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_SRC|GPUTextureUsage.COPY_DST,r=()=>this.device.createTexture({size:[i.tileW,i.tileH,i.layers],format:"rgba16float",usage:a}),o={dimension:"2d-array",arrayLayerCount:i.layers},s=r(),l=r(),f=s.createView(o),u=l.createView(o);s.label=`2D texA ${t}x${e}x${i.layers}`,l.label=`2D texB ${t}x${e}x${i.layers}`,f.label="2D:viewA",u.label="2D:viewB",this._tag.set(f,"2D:A"),this._tag.set(u,"2D:B");let p=this._texPairs.size;return this._texPairs.set(p,{texA:s,texB:l,viewA:f,viewB:u,fullWidth:t,fullHeight:e,tileWidth:i.tileW,tileHeight:i.tileH,tilesX:i.tilesX,tilesY:i.tilesY,layers:i.layers,isA:!0,tiles:null,bindGroupDirty:!0}),this._tid===null&&this.setActiveTexture(p),p}createShaderTextures(t,e){this._tid!==null&&this._texPairs.has(this._tid)&&this.destroyTexturePair(this._tid);let i=this._create2DPair(t,e);return this.setActiveTexture(i),i}destroyTexturePair(t){let e=this._texPairs.get(t);if(e){try{e.texA.destroy()}catch{}try{e.texB.destroy()}catch{}if(Array.isArray(e.tiles))for(let i of e.tiles){if(Array.isArray(i.frames))for(let a of i.frames)try{a.destroy()}catch{}if(i.posBuf&&i.posBuf!==this.nullPosBuffer)try{i.posBuf.destroy()}catch{}}this._texPairs.delete(t),this._tid===t&&(this._tid=null,this.inputTextureView=null,this.outputTextureView=null,this.viewA=null,this.viewB=null)}}destroyAllTexturePairs(){let t=Array.from(this._texPairs.keys());for(let e of t)this.destroyTexturePair(e)}setActiveTexture(t){if(!this._texPairs.has(t))throw new Error("setActiveTexture: invalid id");this._tid=t;let e=this._texPairs.get(t);this.viewA=e.viewA,this.viewB=e.viewB,this.width=e.tileWidth,this.height=e.tileHeight,this.layers=e.layers,this.inputTextureView=e.isA?e.viewA:e.viewB,this.outputTextureView=e.isA?e.viewB:e.viewA}_buildPosBuffer(t,e,i){if((this.useCustomPos|0)===0&&!i)return this.nullPosBuffer;let a=t*e,r=i instanceof Float32Array&&i.length===a*4?i:new Float32Array(a*4),o=this.device.createBuffer({size:r.byteLength,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST});return this.queue.writeBuffer(o,0,r.buffer,r.byteOffset,r.byteLength),o}_writeFrameUniform(t,e){let i=new ArrayBuffer(64),a=new DataView(i);a.setUint32(0,e.fullWidth>>>0,!0),a.setUint32(4,e.fullHeight>>>0,!0),a.setUint32(8,e.tileWidth>>>0,!0),a.setUint32(12,e.tileHeight>>>0,!0),a.setInt32(16,e.originX|0,!0),a.setInt32(20,e.originY|0,!0),a.setInt32(24,e.originZ|0,!0),a.setUint32(28,e.fullDepth>>>0,!0),a.setUint32(32,e.tileDepth>>>0,!0),a.setInt32(36,e.layerIndex|0,!0),a.setUint32(40,e.layers>>>0,!0),a.setUint32(44,0,!0),a.setFloat32(48,e.originXf??0,!0),a.setFloat32(52,e.originYf??0,!0),a.setFloat32(56,0,!0),a.setFloat32(60,0,!0),this.queue.writeBuffer(t,0,i)}_create2DTileBindGroups(t,e={}){let i=this._texPairs.get(t);if(!i)throw new Error("_create2DTileBindGroups: invalid tid");if(Array.isArray(i.tiles)&&!i.bindGroupDirty&&!e.customData)return;let a=[];for(let r=0;r<i.tilesY;r++)for(let o=0;o<i.tilesX;o++){let s=r*i.tilesX+o,l=o*i.tileWidth,f=r*i.tileHeight,u=i.tiles&&i.tiles[s]||null,p;u&&u.posBuf&&!e.customData?p=u.posBuf:p=this._buildPosBuffer(i.tileWidth,i.tileHeight,e.customData);let c;u&&u.frames&&u.frames[0]?c=u.frames[0]:c=this.device.createBuffer({size:64,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});let y=Number.isFinite(e.frameFullWidth)?e.frameFullWidth>>>0:i.fullWidth,m=Number.isFinite(e.frameFullHeight)?e.frameFullHeight>>>0:i.fullHeight,d=y/i.fullWidth,g=m/i.fullHeight,z=l*d,_=f*g;this._writeFrameUniform(c,{fullWidth:y,fullHeight:m,tileWidth:i.tileWidth,tileHeight:i.tileHeight,originX:l,originY:f,originZ:0,fullDepth:1,tileDepth:1,layerIndex:s,layers:i.layers,originXf:z,originYf:_});let w=u?.bgs?.[0]?.bgA??null,x=u?.bgs?.[0]?.bgB??null;if(!w||!x||i.bindGroupDirty)try{w=this.device.createBindGroup({layout:this.bindGroupLayout,entries:[{binding:0,resource:{buffer:this.optionsBuffer}},{binding:1,resource:{buffer:this.paramsBuffer}},{binding:2,resource:{buffer:this.permBuffer}},{binding:3,resource:i.viewA},{binding:4,resource:i.viewB},{binding:5,resource:{buffer:p}},{binding:6,resource:{buffer:c}},{binding:7,resource:this._dummy3D_sampleView},{binding:8,resource:this._dummy3D_writeView}]}),x=this.device.createBindGroup({layout:this.bindGroupLayout,entries:[{binding:0,resource:{buffer:this.optionsBuffer}},{binding:1,resource:{buffer:this.paramsBuffer}},{binding:2,resource:{buffer:this.permBuffer}},{binding:3,resource:i.viewB},{binding:4,resource:i.viewA},{binding:5,resource:{buffer:p}},{binding:6,resource:{buffer:c}},{binding:7,resource:this._dummy3D_sampleView},{binding:8,resource:this._dummy3D_writeView}]})}catch(b){throw new Error(`_create2DTileBindGroups: createBindGroup failed: ${b?.message||b}`)}a.push({layerIndex:s,originX:l,originY:f,frames:[c],posBuf:p,bgs:[{bgA:w,bgB:x}]})}i.tiles=a,i.bindGroupDirty=!1,this._tid===t&&(this._tiles=a)}async _runPipelines(t,e,i,a,r,o,s=1){let l=t,f=e,u=Array.isArray(o),p=0,c=this.device.createCommandEncoder(),y=c.beginComputePass();for(let m of this.noiseChoices){let d=typeof m=="number"?this.entryPoints[m]:m,g=this.pipelines.get(d);g||(g=this.device.createComputePipeline({layout:this.pipelineLayout,compute:{module:this.shaderModule,entryPoint:d}}),this.pipelines.set(d,g)),u&&this.setNoiseParams(o[p++]),y.setPipeline(g),y.setBindGroup(0,l),y.dispatchWorkgroups(Math.ceil(i/8),Math.ceil(a/8),s),[l,f]=[f,l]}return y.end(),this.queue.submit([c.finish()]),f}async computeToTexture(t,e,i={},a={}){let r=t|0,o=e|0;if(!(r>0&&o>0))throw new Error(`computeToTexture: invalid size ${t}x${e}`);this._tid==null&&this._create2DPair(r,o);let s=this._texPairs.get(this._tid);if(!s||s.fullWidth!==r||s.fullHeight!==o){let y=this._create2DPair(r,o);this.setActiveTexture(y),s=this._texPairs.get(y)}i&&!Array.isArray(i)&&this.setNoiseParams(i);let l=a||{};this.setOptions({...l,ioFlags:0,useCustomPos:l.useCustomPos??this.useCustomPos}),(!s.tiles||s.bindGroupDirty||l.customData)&&this._create2DTileBindGroups(this._tid,a);let f=s.isA,u=null,p=null;for(let y of s.tiles){let{bgs:m}=y,{bgA:d,bgB:g}=m[0],z=u?u===d?d:g:f?d:g,_=z===d?g:d;u=await this._runPipelines(z,_,s.tileWidth,s.tileHeight,1,i,1),p={bgA:d,bgB:g}}let c=u===p.bgB;return s.isA=c,this.isA=c,this.setActiveTexture(this._tid),this.getCurrentView()}getCurrentView(){let t=this._texPairs.get(this._tid);return t?t.isA?t.viewA:t.viewB:null}_compute3DTiling(t,e,i){let a=Math.min(t,T),r=Math.min(e,T),o=this.device?.limits?.maxBufferSize??256*1024*1024,s=a*r*$,l=Math.max(1,Math.floor(o*.8/Math.max(1,s))),f=Math.min(i,T,l),u=Math.ceil(t/a),p=Math.ceil(e/r),c=Math.ceil(i/f);return{tw:a,th:r,td:f,nx:u,ny:p,nz:c}}_create3DChunks(t,e,i){let a=this._compute3DTiling(t,e,i),r=[],o=GPUTextureUsage.STORAGE_BINDING|GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_SRC|GPUTextureUsage.COPY_DST;for(let s=0;s<a.nz;s++)for(let l=0;l<a.ny;l++)for(let f=0;f<a.nx;f++){let u=f*a.tw,p=l*a.th,c=s*a.td,y=this.device.createTexture({size:{width:a.tw,height:a.th,depthOrArrayLayers:a.td},dimension:"3d",format:"rgba16float",usage:o}),m=this.device.createTexture({size:{width:a.tw,height:a.th,depthOrArrayLayers:a.td},dimension:"3d",format:"rgba16float",usage:o}),d=y.createView({dimension:"3d"}),g=m.createView({dimension:"3d"});y.label=`3D texA ${a.tw}x${a.th}x${a.td} @ (${f},${l},${s})`,m.label=`3D texB ${a.tw}x${a.th}x${a.td} @ (${f},${l},${s})`,d.label=`3D:viewA[${f},${l},${s}]`,g.label=`3D:viewB[${f},${l},${s}]`,this._tag.set(d,`3D:A[${f},${l},${s}]`),this._tag.set(g,`3D:B[${f},${l},${s}]`),r.push({texA:y,texB:m,viewA:d,viewB:g,ox:u,oy:p,oz:c,w:a.tw,h:a.th,d:a.td,isA:!0,fb:null,posBuf:null,bgA:null,bgB:null})}return{chunks:r,tile:{w:a.tw,h:a.th,d:a.td},full:{w:t,h:e,d:i},grid:{nx:a.nx,ny:a.ny,nz:a.nz}}}_destroy3DSet(t){if(t)for(let e of t.chunks){try{e.texA.destroy()}catch{}try{e.texB.destroy()}catch{}if(e.viewA=null,e.viewB=null,e.bgA=null,e.bgB=null,e.fb){try{e.fb.destroy()}catch{}e.fb=null}if(e.posBuf&&e.posBuf!==this.nullPosBuffer){try{e.posBuf.destroy()}catch{}e.posBuf=null}}}destroyAllVolumes(){for(let[t,e]of this._volumeCache)this._destroy3DSet(e),this._volumeCache.delete(t)}get3DView(t){let e=this._volumeCache.get(String(t));if(!e)return null;let i=e.chunks.map(a=>a.isA?a.viewA:a.viewB);return i.length===1?i[0]:{views:i,meta:{full:e.full,tile:e.tile,grid:e.grid}}}destroyVolume(t){let e=String(t),i=this._volumeCache.get(e);i&&(this._destroy3DSet(i),this._volumeCache.delete(e))}_getOrCreate3DVolume(t,e,i,a=null,r=null){let o=a?String(a):`${t}x${e}x${i}`,s=this._volumeCache.get(o);if(s)return s;s=this._create3DChunks(t,e,i);for(let l of s.chunks){l.fb=this.device.createBuffer({size:64,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});let f=r&&Number.isFinite(r?.w)?r.w>>>0:s.full.w,u=r&&Number.isFinite(r?.h)?r.h>>>0:s.full.h,p=r&&Number.isFinite(r?.d)?r.d>>>0:s.full.d,c=f/s.full.w,y=u/s.full.h,m=l.ox*c,d=l.oy*y;this._writeFrameUniform(l.fb,{fullWidth:f,fullHeight:u,tileWidth:l.w,tileHeight:l.h,originX:l.ox,originY:l.oy,originZ:l.oz,fullDepth:p,tileDepth:l.d,layerIndex:0,layers:1,originXf:m,originYf:d});let g=this._buildPosBuffer(l.w,l.h,null);l.posBuf=g;try{l.bgA=this.device.createBindGroup({layout:this.bindGroupLayout,entries:[{binding:0,resource:{buffer:this.optionsBuffer}},{binding:1,resource:{buffer:this.paramsBuffer}},{binding:2,resource:{buffer:this.permBuffer}},{binding:3,resource:this._dummy2D_sampleView},{binding:4,resource:this._dummy2D_writeView},{binding:5,resource:{buffer:g}},{binding:6,resource:{buffer:l.fb}},{binding:7,resource:l.viewA},{binding:8,resource:l.viewB}]}),l.bgB=this.device.createBindGroup({layout:this.bindGroupLayout,entries:[{binding:0,resource:{buffer:this.optionsBuffer}},{binding:1,resource:{buffer:this.paramsBuffer}},{binding:2,resource:{buffer:this.permBuffer}},{binding:3,resource:this._dummy2D_sampleView},{binding:4,resource:this._dummy2D_writeView},{binding:5,resource:{buffer:l.posBuf}},{binding:6,resource:{buffer:l.fb}},{binding:7,resource:l.viewB},{binding:8,resource:l.viewA}]})}catch(z){throw new Error(`_getOrCreate3DVolume: createBindGroup failed: ${z?.message||z}`)}}return s._bindGroupsDirty=!1,this._volumeCache.set(o,s),s}_recreate3DBindGroups(t,e=null){if(!t||!Array.isArray(t.chunks))return;let i=e&&Number.isFinite(e.w)?e.w>>>0:t.full.w,a=e&&Number.isFinite(e.h)?e.h>>>0:t.full.h,r=e&&Number.isFinite(e.d)?e.d>>>0:t.full.d;for(let o of t.chunks){if(!o.fb){o.fb=this.device.createBuffer({size:64,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});let f=i/t.full.w,u=a/t.full.h,p=o.ox*f,c=o.oy*u;this._writeFrameUniform(o.fb,{fullWidth:i,fullHeight:a,tileWidth:o.w,tileHeight:o.h,originX:o.ox,originY:o.oy,originZ:o.oz,fullDepth:r,tileDepth:o.d,layerIndex:0,layers:1,originXf:p,originYf:c})}o.posBuf||(o.posBuf=this._buildPosBuffer(o.w,o.h,null));let s=[{binding:0,resource:{buffer:this.optionsBuffer}},{binding:1,resource:{buffer:this.paramsBuffer}},{binding:2,resource:{buffer:this.permBuffer}},{binding:3,resource:this._dummy2D_sampleView},{binding:4,resource:this._dummy2D_writeView},{binding:5,resource:{buffer:o.posBuf}},{binding:6,resource:{buffer:o.fb}},{binding:7,resource:o.viewA},{binding:8,resource:o.viewB}],l=[{binding:0,resource:{buffer:this.optionsBuffer}},{binding:1,resource:{buffer:this.paramsBuffer}},{binding:2,resource:{buffer:this.permBuffer}},{binding:3,resource:this._dummy2D_sampleView},{binding:4,resource:this._dummy2D_writeView},{binding:5,resource:{buffer:o.posBuf}},{binding:6,resource:{buffer:o.fb}},{binding:7,resource:o.viewB},{binding:8,resource:o.viewA}];try{o.bgA=this.device.createBindGroup({layout:this.bindGroupLayout,entries:s}),o.bgB=this.device.createBindGroup({layout:this.bindGroupLayout,entries:l})}catch(f){throw new Error(`_recreate3DBindGroups: failed to create bind groups: ${f?.message||f}`)}}t._bindGroupsDirty=!1}async computeToTexture3D(t,e,i,a={},r={}){let o=t|0,s=e|0,l=i|0;if(!(o>0&&s>0&&l>0))throw new Error(`computeToTexture3D: invalid size ${t}x${e}x${i}`);a&&!Array.isArray(a)&&this.setNoiseParams(a);let f=r||{};this.setOptions({...f,ioFlags:3,useCustomPos:f.useCustomPos??this.useCustomPos});let u=r&&(Number.isFinite(r.frameFullWidth)||Number.isFinite(r.frameFullHeight)||Number.isFinite(r.frameFullDepth))?{w:Number.isFinite(r.frameFullWidth)?r.frameFullWidth>>>0:o,h:Number.isFinite(r.frameFullHeight)?r.frameFullHeight>>>0:s,d:Number.isFinite(r.frameFullDepth)?r.frameFullDepth>>>0:l}:null,p=this._getOrCreate3DVolume(o,s,l,r.id,u);if(!p)throw new Error("computeToTexture3D: failed to create or retrieve volume");(p._bindGroupsDirty||!p.chunks[0].bgA||!p.chunks[0].bgB)&&this._recreate3DBindGroups(p,u);let c=null;for(let m of p.chunks){let d=m.isA?m.bgA:m.bgB,g=m.isA?m.bgB:m.bgA;if(!d||!g)throw new Error("computeToTexture3D: missing bind groups (volume not initialized correctly)");c=await this._runPipelines(d,g,m.w,m.h,m.d,a,m.d),m.isA=c===m.bgB}let y=p.chunks.map(m=>m.isA?m.viewA:m.viewB);return y.length===1?y[0]:{views:y,meta:{full:p.full,tile:p.tile,grid:p.grid}}}configureCanvas(t){let e=navigator.gpu.getPreferredCanvasFormat&&navigator.gpu.getPreferredCanvasFormat()||"bgra8unorm",i=t.getContext("webgpu");i.configure({device:this.device,format:e,alphaMode:"opaque",size:[t.width,t.height]}),this._ctxMap.set(t,{ctx:i,size:[t.width,t.height]})}initBlitRender(){this.sampler||(this.sampler=this.device.createSampler({magFilter:"linear",minFilter:"linear",addressModeU:"clamp-to-edge",addressModeV:"clamp-to-edge"})),this.bgl2D||(this.bgl2D=this.device.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.FRAGMENT,sampler:{}},{binding:1,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"float",viewDimension:"2d-array"}},{binding:2,visibility:GPUShaderStage.FRAGMENT,buffer:{type:"uniform"}}]}),this.pipeline2D=this.device.createRenderPipeline({layout:this.device.createPipelineLayout({bindGroupLayouts:[this.bgl2D]}),vertex:{module:this.device.createShaderModule({code:A}),entryPoint:"vs_main"},fragment:{module:this.device.createShaderModule({code:A}),entryPoint:"fs_main",targets:[{format:"bgra8unorm"}]},primitive:{topology:"triangle-list"}}),this.blit2DUbo=this.device.createBuffer({size:16,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST})),this.bgl3D||(this.bgl3D=this.device.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.FRAGMENT,sampler:{}},{binding:1,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"float",viewDimension:"3d"}},{binding:2,visibility:GPUShaderStage.FRAGMENT,buffer:{type:"uniform"}}]}),this.pipeline3D=this.device.createRenderPipeline({layout:this.device.createPipelineLayout({bindGroupLayouts:[this.bgl3D]}),vertex:{module:this.device.createShaderModule({code:C}),entryPoint:"vs_main"},fragment:{module:this.device.createShaderModule({code:C}),entryPoint:"fs_main",targets:[{format:"bgra8unorm"}]},primitive:{topology:"triangle-list"}}),this.blit3DUbo=this.device.createBuffer({size:16,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}))}_renderCommonCanvasSetup(t,e){let i="bgra8unorm",a=this._ctxMap.get(t);if(a){let s=t.width|0,l=t.height|0;(a.size[0]!==s||a.size[1]!==l)&&(a.size=[s,l],a.ctx.configure({device:this.device,format:i,alphaMode:"opaque",size:a.size}))}else{let s=t.getContext("webgpu"),l=[t.width|0,t.height|0];s.configure({device:this.device,format:i,alphaMode:"opaque",size:l}),a={ctx:s,size:l},this._ctxMap.set(t,a)}let r=this.device.createCommandEncoder(),o=r.beginRenderPass({colorAttachments:[{view:a.ctx.getCurrentTexture().createView(),loadOp:e?"clear":"load",clearValue:{r:0,g:0,b:0,a:1},storeOp:"store"}]});return{enc:r,pass:o,ctxEntry:a}}renderTextureToCanvas(t,e,i={}){let{layer:a=0,channel:r=0,preserveCanvasSize:o=!0,clear:s=!0}=i;if(this.initBlitRender(),!o)try{let c=t.texture;c&&typeof c.width=="number"&&typeof c.height=="number"&&(e.width=c.width,e.height=c.height)}catch{}let l=new Uint32Array([a>>>0,r>>>0,0,0]);this.queue.writeBuffer(this.blit2DUbo,0,l.buffer,l.byteOffset,l.byteLength);let f=this.device.createBindGroup({layout:this.bgl2D,entries:[{binding:0,resource:this.sampler},{binding:1,resource:t},{binding:2,resource:{buffer:this.blit2DUbo}}]}),{enc:u,pass:p}=this._renderCommonCanvasSetup(e,s);p.setPipeline(this.pipeline2D),p.setBindGroup(0,f),p.draw(6,1,0,0),p.end(),this.queue.submit([u.finish()])}renderTexture3DSliceToCanvas(t,e,i={}){let{depth:a,slice:r=0,zNorm:o=null,channel:s=0,chunk:l=0,preserveCanvasSize:f=!0,clear:u=!0}=i;this.initBlitRender();let p,c;if(t&&t.views&&Array.isArray(t.views)?(p=t.views[Math.max(0,Math.min(l|0,t.views.length-1))],c=t.meta?.tile?.d??a):(p=t,c=a),!p||!c)throw new Error("renderTexture3DSliceToCanvas: need a 3D view and its depth");if(!f)try{let w=p.texture;w&&typeof w.width=="number"&&typeof w.height=="number"&&(e.width=w.width,e.height=w.height)}catch{}let y=o??(Math.min(Math.max(r,0),c-1)+.5)/c;y=Math.min(Math.max(y,0),1);let m=new ArrayBuffer(16),d=new DataView(m);d.setFloat32(0,y,!0),d.setUint32(4,s>>>0,!0),d.setUint32(8,0,!0),d.setUint32(12,0,!0),this.queue.writeBuffer(this.blit3DUbo,0,m);let g=this.device.createBindGroup({layout:this.bgl3D,entries:[{binding:0,resource:this.sampler},{binding:1,resource:p},{binding:2,resource:{buffer:this.blit3DUbo}}]}),{enc:z,pass:_}=this._renderCommonCanvasSetup(e,u);_.setPipeline(this.pipeline3D),_.setBindGroup(0,g),_.draw(6,1,0,0),_.end(),this.queue.submit([z.finish()])}},R=class{constructor(t=Date.now()){t<1e7&&(t*=1e7),this.seedN=t,this.seedK=t,this.perm=new Uint8Array(512),this.seed(t)}seed(t){let e=this.xorshift(t);for(let i=0;i<256;i++)this.perm[i]=i;for(let i=255;i>0;i--){let a=Math.floor(e()*(i+1));[this.perm[i],this.perm[a]]=[this.perm[a],this.perm[i]]}for(let i=0;i<256;i++)this.perm[i+256]=this.perm[i]}setSeed(t){this.seedN=t,this.seed(t),this.resetSeed()}random(t,e,i){let a;return typeof i=="number"?a=this.perm[(t&255)+this.perm[(e&255)+this.perm[i&255]]]&255:a=this.perm[(t&255)+this.perm[e&255]]&255,this.perm[a]/255*2-1}seededRandom(){this.seedK+=Math.E;let t=1e9*Math.sin(this.seedK);return t-Math.floor(t)}resetSeed(){this.seedK=this.seedN}xorshift(t){let e=t;return function(){return e^=e<<13,e^=e>>17,e^=e<<5,(e<0?1+~e:e)/4294967295}}dot(t,e=0,i=0,a=0){return t[0]*e+t[1]*i+t[2]*a}};document.body.insertAdjacentHTML("afterbegin",G);var E={0:"Perlin",1:"Billow",2:"AntiBillow",3:"Ridge",4:"AntiRidge",5:"RidgedMultifractal",6:"RidgedMultifractal2",7:"RidgedMultifractal3",8:"RidgedMultifractal4",9:"AntiRidgedMultifractal",10:"AntiRidgedMultifractal2",11:"AntiRidgedMultifractal3",12:"AntiRidgedMultifractal4",13:"FBM",14:"FBM2",15:"FBM3",16:"CellularBM1",17:"CellularBM2",18:"CellularBM3",19:"VoronoiBM1",20:"VoronoiBM2",21:"VoronoiBM3",22:"CellularPattern",23:"WorleyPattern",24:"AntiCellularPattern",25:"AntiWorleyPattern",26:"LanczosBillow",27:"LanczosAntiBillow",28:"VoronoiTileNoise",29:"VoronoiCircleNoise",30:"VoronoiCircle2",31:"VoronoiFlatShade",32:"VoronoiRipple3D",33:"VoronoiRipple3D2",34:"VoronoiCircularRipple",35:"FVoronoiRipple3D",36:"FVoronoiCircularRipple",37:"RippleNoise",38:"FractalRipples",39:"HexWorms",40:"PerlinWorms",41:"White Noise",42:"Blue Noise",43:"Simplex",44:"Curl2D",45:"CurlFBM2D",46:"DomainWarpFBM1",47:"DomainWarpFBM2",48:"GaborAnisotropic",49:"TerraceNoise",50:"FoamNoise",51:"Turbulence",52:"Perlin4D",53:"Worley4D",54:"AntiWorley4D"},J={3:{clamp:{freq:[.25,8],gain:[.2,.8],octaves:[1,12]}},4:{clamp:{freq:[.25,8],gain:[.2,.8],octaves:[1,12]}},5:{clamp:{freq:[.25,8],gain:[.2,.9],octaves:[2,14]}},6:{clamp:{freq:[.25,8],gain:[.2,.9],octaves:[2,14]}},7:{clamp:{freq:[.25,8],gain:[.2,.9],octaves:[2,14]}},8:{clamp:{freq:[.25,8],gain:[.2,.9],octaves:[2,14]}},13:{clamp:{gain:[.2,.8],octaves:[2,10]}},14:{clamp:{gain:[.2,.8],octaves:[2,10]}},15:{clamp:{gain:[.2,.8],octaves:[2,10]}},19:{clamp:{threshold:[0,1]}},20:{clamp:{threshold:[0,1]}},21:{clamp:{threshold:[0,1]}},22:{clamp:{threshold:[0,1]}},23:{clamp:{threshold:[0,1]}},24:{clamp:{threshold:[0,1]}},25:{clamp:{threshold:[0,1]}},44:{force:{turbulence:1},clamp:{warpAmp:[.1,2],freq:[.25,6]}},45:{force:{turbulence:1},clamp:{warpAmp:[.1,2],freq:[.25,6]}},46:{force:{turbulence:1},clamp:{warpAmp:[.1,3]}},47:{force:{turbulence:1},clamp:{warpAmp:[.1,3]}},48:{clamp:{gaborRadius:[.5,6]}},51:{force:{turbulence:1},clamp:{gain:[.5,.95]}}},Q=["clearTexture","computePerlin4D","computeWorley4D"],S=128,ee="toroidalDemo",M=new Map;function D(n,t,e,i){if(!Object.prototype.hasOwnProperty.call(n,t))return;let a=Number(n[t]);if(!Number.isFinite(a))return;let r=Number(e),o=Number(i);n[t]=Math.min(Math.max(a,r),o)}function ie(n,t){let e={...t},i=J[n];if(i&&i.clamp){let r=i.clamp;r.freq&&D(e,"freq",r.freq[0],r.freq[1]),r.gain&&D(e,"gain",r.gain[0],r.gain[1]),r.octaves&&D(e,"octaves",r.octaves[0],r.octaves[1]),r.threshold&&D(e,"threshold",r.threshold[0],r.threshold[1]),r.warpAmp&&D(e,"warpAmp",r.warpAmp[0],r.warpAmp[1]),r.gaborRadius&&D(e,"gaborRadius",r.gaborRadius[0],r.gaborRadius[1])}if(i&&i.force)for(let[r,o]of Object.entries(i.force))e[r]=o;let a=M.get(n);if(a)for(let[r,o]of Object.entries(a))typeof o=="number"&&Number.isFinite(o)&&(e[r]=o);return e}function U(){let n=(e,i)=>{let a=document.getElementById(e);if(!a)return i;let r=Number(a.value);return Number.isFinite(r)?r:i};return{seed:Math.max(1,Math.floor(n("noise-seed",1234567890))),zoom:n("noise-zoom",1),freq:n("noise-freq",1),octaves:Math.max(1,Math.floor(n("noise-octaves",8))),lacunarity:n("noise-lacunarity",2),gain:n("noise-gain",.5),xShift:n("noise-xShift",0),yShift:n("noise-yShift",0),zShift:n("noise-zShift",0),threshold:n("noise-threshold",.1),turbulence:0,seedAngle:0,exp1:1,exp2:0,rippleFreq:10,time:0,warpAmp:.5,gaborRadius:4,terraceStep:8,toroidal:0,voroMode:0,edgeK:0}}function te(){let n=document.querySelectorAll('input[type="checkbox"][name="noise-type"]'),t=[];return n.forEach(e=>{if(e.checked){let i=Number(e.dataset.bit);Number.isInteger(i)&&t.push(i)}}),t}function ae(){let n=document.getElementById("noise-canvas"),t=document.getElementById("view-stack");if(!n&&t&&(n=document.createElement("canvas"),n.id="noise-canvas",n.width=800,n.height=800,t.appendChild(n)),!n)throw new Error("Missing main preview canvas (#noise-canvas)");let e=document.getElementById("mosaic");if(!e)throw new Error("Missing #mosaic container");let i=[],a=e.querySelectorAll("canvas");if(a.length)a.forEach(r=>i.push(r));else for(let r=0;r<9;r++){let o=document.createElement("canvas");o.width=256,o.height=256,e.appendChild(o),i.push(o)}return{mainCanvas:n,mosaicCanvases:i}}function re(n){return n.length?n.map(e=>E[e]||String(e)).join(", "):"Perlin"}function oe(){let n=document.getElementById("override-mode");if(!n)return;n.innerHTML="";let t=Object.keys(E).map(e=>Number(e)).filter(e=>Number.isInteger(e)&&e>=0&&e<=54).sort((e,i)=>e-i);for(let e of t){let i=document.createElement("option");i.value=String(e),i.textContent=`${e}: ${E[e]}`,n.appendChild(i)}t.length&&(n.value=String(t[0]))}function q(n){let t=M.get(n)||{},e=(i,a)=>{let r=document.getElementById(i);if(!r)return;let o=t[a];r.value=typeof o=="number"&&Number.isFinite(o)?String(o):""};e("ov-zoom","zoom"),e("ov-freq","freq"),e("ov-gain","gain"),e("ov-octaves","octaves"),e("ov-warp","warpAmp"),e("ov-threshold","threshold"),e("ov-gabor","gaborRadius"),e("ov-xShift","xShift"),e("ov-yShift","yShift"),e("ov-zShift","zShift")}function le(){let n=document.getElementById("override-mode");if(!n)return;let t=Number(n.value);if(!Number.isInteger(t))return;let e=m=>{let d=document.getElementById(m);if(!d)return null;let g=String(d.value).trim();if(!g)return null;let z=Number(g);return Number.isFinite(z)?z:null},i={},a=e("ov-zoom"),r=e("ov-freq"),o=e("ov-gain"),s=e("ov-octaves"),l=e("ov-warp"),f=e("ov-threshold"),u=e("ov-gabor"),p=e("ov-xShift"),c=e("ov-yShift"),y=e("ov-zShift");a!==null&&(i.zoom=a),r!==null&&(i.freq=r),o!==null&&(i.gain=o),s!==null&&(i.octaves=s),l!==null&&(i.warpAmp=l),f!==null&&(i.threshold=f),u!==null&&(i.gaborRadius=u),p!==null&&(i.xShift=p),c!==null&&(i.yShift=c),y!==null&&(i.zShift=y),Object.keys(i).length?M.set(t,i):M.delete(t)}function se(){let n=document.getElementById("z-slice"),t=document.getElementById("z-slice-num"),e=0;return n?e=Number(n.value):t&&(e=Number(t.value)),Number.isFinite(e)||(e=0),e=Math.min(Math.max(Math.round(e),0),S-1),n&&String(n.value)!==String(e)&&(n.value=String(e)),t&&String(t.value)!==String(e)&&(t.value=String(e)),e}async function N(n,t){let e=Number(document.getElementById("res-width")?.value)||800,i=Number(document.getElementById("res-height")?.value)||800;t.width=e,t.height=i;let a=document.getElementById("preview-meta"),r=document.getElementById("preview-stats"),o=U();n.buildPermTable(o.seed|0);let s=te(),l=s.length?s:[0],f={getGradient:0,outputChannel:1,baseRadius:0,heightScale:1,useCustomPos:0},u=performance.now();await n.computeToTexture(e,i,o,{...f,noiseChoices:["clearTexture"],frameFullWidth:e,frameFullHeight:i});for(let d of l){let g=ie(d,o);await n.computeToTexture(e,i,g,{...f,noiseChoices:[d],frameFullWidth:e,frameFullHeight:i})}let p=performance.now(),c=n.getCurrentView(),y=performance.now();c&&n.renderTextureToCanvas(c,t,{layer:0,channel:0,preserveCanvasSize:!0,clear:!0});let m=performance.now();if(a&&(a.textContent=`Height field preview \xB7 ${e}\xD7${i} \xB7 modes: ${re(l)}`),r){let d=(p-u).toFixed(1),g=(m-y).toFixed(1);r.textContent=`GPU compute ${d} ms \xB7 blit ${g} ms`}return{resW:e,resH:i,noiseBits:l}}function W(n,t,e){if(!t)return;let i=S,r=(se()+.5)/i,o=e.length||9;for(let s=0;s<o;s++){let l=e[s];l.width=S,l.height=S,n.renderTexture3DSliceToCanvas(t,l,{depth:i,zNorm:r,channel:0,chunk:0,preserveCanvasSize:!0,clear:!0})}}async function F(n,t,e){let a={...U(),toroidal:1},r=performance.now(),o=await n.computeToTexture3D(S,S,S,a,{noiseChoices:Q,outputChannel:1,id:ee}),s=performance.now();e.lastToroidalVolumeView=o,e.lastToroidalComputeMs=s-r,W(n,o,t)}async function ne(){let n=document.getElementById("preview-stats");if(!navigator.gpu){console.error("WebGPU not available in this browser."),n&&(n.textContent="WebGPU not available in this browser.");return}let t=await navigator.gpu.requestAdapter();if(!t){console.error("Failed to get GPU adapter."),n&&(n.textContent="Failed to get GPU adapter.");return}let e=await t.requestDevice(),i=new B(e,e.queue),{mainCanvas:a,mosaicCanvases:r}=ae();i.configureCanvas(a),r.forEach(x=>i.configureCanvas(x)),oe();let o=document.getElementById("override-mode");if(o){let x=Number(o.value);Number.isInteger(x)&&q(x)}let s=["ov-zoom","ov-freq","ov-gain","ov-octaves","ov-warp","ov-threshold","ov-gabor","ov-xShift","ov-yShift","ov-zShift"],l=!1,f=()=>{l||(l=!0,requestAnimationFrame(()=>{l=!1,N(i,a).catch(x=>{console.error(x),n&&(n.textContent=String(x))})}))};s.forEach(x=>{let b=document.getElementById(x);b&&b.addEventListener("change",()=>{le(),f()})}),o&&o.addEventListener("change",()=>{let x=Number(o.value);Number.isInteger(x)&&q(x)});let u=document.getElementById("ov-clear");u&&u.addEventListener("click",()=>{let x=document.getElementById("override-mode");if(!x)return;let b=Number(x.value);Number.isInteger(b)&&(M.delete(b),q(b),f())});let p=document.getElementById("noise-overrides-btn");p&&p.addEventListener("click",()=>{let x=document.getElementById("overrides-group");x&&x.scrollIntoView({behavior:"smooth",block:"start"})});let c={lastToroidalVolumeView:null,lastToroidalComputeMs:0},y=document.getElementById("render-btn"),m=document.getElementById("apply-res");y&&y.addEventListener("click",()=>{N(i,a).then(()=>F(i,r,c)).catch(x=>{console.error(x),n&&(n.textContent=String(x))})}),m&&m.addEventListener("click",()=>{f()});let d=document.getElementById("z-slice"),g=document.getElementById("z-slice-num"),z=()=>{c.lastToroidalVolumeView&&W(i,c.lastToroidalVolumeView,r)};d&&d.addEventListener("input",()=>{let x=Number(d.value);g&&(g.value=String(x)),z()}),g&&g.addEventListener("change",()=>{let x=Number(g.value);Number.isFinite(x)||(x=0),x=Math.min(Math.max(Math.round(x),0),S-1),g.value=String(x),d&&(d.value=String(x)),z()}),["noise-seed","noise-zoom","noise-freq","noise-octaves","noise-lacunarity","noise-gain","noise-xShift","noise-yShift","noise-zShift","noise-threshold"].forEach(x=>{let b=document.getElementById(x);b&&(b.addEventListener("input",f),b.addEventListener("change",f))}),document.querySelectorAll('input[type="checkbox"][name="noise-type"]').forEach(x=>{x.addEventListener("change",f)}),await N(i,a),await F(i,r,c)}document.addEventListener("DOMContentLoaded",()=>{ne().catch(n=>console.error(n))});})();
