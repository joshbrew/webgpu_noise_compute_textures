(()=>{var xe=`<!-- tools/noise/noiseComponent.html -->\r
<div id="noise-app">\r
  <style>\r
    :root {\r
...
`;var he=`const PI : f32 = 3.141592653589793;\r
...
`;var ae=`// Fullscreen quad (module-scope constant)\r
...
`;var oe=`const kQuad : array<vec2<f32>, 6> = array<vec2<f32>, 6>(\r
  vec2<f32>(-1.0, -1.0), vec2<f32>( 1.0, -1.0), vec2<f32>(-1.0,  1.0),\r
  vec2<f32>(-1.0,  1.0), vec2<f32>( 1.0, -1.0), vec2<f32>( 1.0,  1.0)\r
);\r
\r
...
`;var ye=8192,ne=2048,Re=8,ie=class{constructor(t,e){this.device=t,this.queue=e,this.maxBufferChunkBytes=8e6,this.entryPoints=["computePerlin","computeBillow","computeAntiBillow","computeRidge","computeAntiRidge","computeRidgedMultifractal","computeRidgedMultifractal2","computeRidgedMultifractal3","computeRidgedMultifractal4","computeAntiRidgedMultifractal","computeAntiRidgedMultifractal2","computeAntiRidgedMultifractal3","computeAntiRidgedMultifractal4","computeFBM","computeFBM2","computeFBM3","computeCellularBM1","computeCellularBM2","computeCellularBM3","computeVoronoiBM1","computeVoronoiBM2","computeVoronoiBM3","computeCellular","computeWorley","computeAntiCellular","computeAntiWorley","computeLanczosBillow","computeLanczosAntiBillow","computeVoronoiTileNoise","computeVoronoiCircleNoise","computeVoronoiCircle2","computeVoronoiFlatShade","computeVoronoiRipple3D","computeVoronoiRipple3D2","computeVoronoiCircularRipple","computeFVoronoiRipple3D","computeFVoronoiCircularRipple","computeRippleNoise","computeFractalRipples","computeHexWorms","computePerlinWorms","computeWhiteNoise","computeBlueNoise","computeSimplex","computeSimplexFBM","computeCurl2D","computeCurlFBM2D","computeDomainWarpFBM1","computeDomainWarpFBM2","computeGaborAnisotropic","computeTerraceNoise","computeFoamNoise","computeTurbulence","computePerlin4D","computeWorley4D","computeAntiWorley4D","computeCellular4D","computeAntiCellular4D","computeBillow4D","computeAntiBillow4D","computeLanczosBillow4D","computeLanczosAntiBillow4D","computeFBM4D","computeVoronoi4D","computeVoronoiBM1_4D","computeVoronoiBM2_4D","computeVoronoiBM3_4D","computeVoronoiBM1_4D_vec","computeVoronoiBM2_4D_vec","computeVoronoiBM3_4D_vec","computeWorleyBM1_4D","computeWorleyBM2_4D","computeWorleyBM3_4D","computeWorleyBM1_4D_vec","computeWorleyBM2_4D_vec","computeWorleyBM3_4D_vec","computeCellularBM1_4D","computeCellularBM2_4D","computeCellularBM3_4D","computeCellularBM1_4D_vec","computeCellularBM2_4D_vec","computeCellularBM3_4D_vec","computeTerraceNoise4D","computeFoamNoise4D","computeTurbulence4D","computeGauss5x5","computeNormal","computeNormal8","computeSphereNormal","computeNormalVolume","clearTexture"],this.shaderModule=t.createShaderModule({code:he}),this.bindGroupLayout=t.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}},{binding:1,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}},{binding:2,visibility:GPUShaderStage.COMPUTE,buffer:{type:"read-only-storage"}},{binding:3,visibility:GPUShaderStage.COMPUTE,texture:{sampleType:"float",viewDimension:"2d-array"}},{binding:4,visibility:GPUShaderStage.COMPUTE,storageTexture:{access:"write-only",format:"rgba16float",viewDimension:"2d-array"}},{binding:5,visibility:GPUShaderStage.COMPUTE,buffer:{type:"read-only-storage"}},{binding:6,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}},{binding:7,visibility:GPUShaderStage.COMPUTE,texture:{sampleType:"float",viewDimension:"3d"}},{binding:8,visibility:GPUShaderStage.COMPUTE,storageTexture:{access:"write-only",format:"rgba16float",viewDimension:"3d"}}]}),this.pipelineLayout=t.createPipelineLayout({bindGroupLayouts:[this.bindGroupLayout]}),this.pipelines=new Map,this._texPairs=new Map,this._tid=null,this._tag=new WeakMap,this._volumeCache=new Map,this.viewA=null,this.viewB=null,this.width=0,this.height=0,this.layers=1,this.isA=!0,this._initBuffers(),this._ensureDummies(),this._ctxMap=new WeakMap}_initBuffers(){this.optionsBuffer?.destroy(),this.paramsBuffer?.destroy(),this.permBuffer?.destroy(),this.nullPosBuffer?.destroy(),this.optionsBuffer=this.device.createBuffer({size:32,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),this.paramsBuffer=this.device.createBuffer({size:88,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),this.permBuffer=this.device.createBuffer({size:512*4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}),this.nullPosBuffer=this.device.createBuffer({size:64,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}),this.queue.writeBuffer(this.optionsBuffer,0,new ArrayBuffer(32)),this.queue.writeBuffer(this.paramsBuffer,0,new ArrayBuffer(88)),this.queue.writeBuffer(this.permBuffer,0,new Uint32Array(512))}_ensureDummies(){this._dummy2D_sampleTex||(this._dummy2D_sampleTex=this.device.createTexture({size:[1,1,1],format:"rgba16float",usage:GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_SRC}),this._dummy2D_sampleView=this._dummy2D_sampleTex.createView({dimension:"2d-array",arrayLayerCount:1})),this._dummy2D_writeTex||(this._dummy2D_writeTex=this.device.createTexture({size:[1,1,1],format:"rgba16float",usage:GPUTextureUsage.STORAGE_BINDING|GPUTextureUsage.COPY_DST}),this._dummy2D_writeView=this._dummy2D_writeTex.createView({dimension:"2d-array",arrayLayerCount:1})),this._dummy3D_sampleTex||(this._dummy3D_sampleTex=this.device.createTexture({size:{width:1,height:1,depthOrArrayLayers:1},dimension:"3d",format:"rgba16float",usage:GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_SRC}),this._dummy3D_sampleView=this._dummy3D_sampleTex.createView({dimension:"3d"})),this._dummy3D_writeTex||(this._dummy3D_writeTex=this.device.createTexture({size:{width:1,height:1,depthOrArrayLayers:1},dimension:"3d",format:"rgba16float",usage:GPUTextureUsage.STORAGE_BINDING|GPUTextureUsage.COPY_DST}),this._dummy3D_writeView=this._dummy3D_writeTex.createView({dimension:"3d"}))}_getMaxBufferChunkBytes(t){let e=this.device?.limits?.maxBufferSize??268435456,i=Math.max(1024*1024,Math.floor(e*.9)),r=Number.isFinite(t)?Math.floor(t):this.maxBufferChunkBytes;return(!Number.isFinite(r)||r<=0)&&(r=this.maxBufferChunkBytes),r=Math.max(4,r)&-4,Math.min(i,r)}_writeBufferChunked(t,e,i,r,a,n=null){let l=a|0;if(!(l>0))return;let s=this._getMaxBufferChunkBytes(n),f=0;for(;f<l;){let p=Math.min(s,l-f)|0;if(p=p&-4,p<=0)break;this.queue.writeBuffer(t,e+f|0,i,r+f|0,p),f=f+p|0}if(f!==l)throw new Error(`_writeBufferChunked: incomplete write ${f}/${l} bytes`)}async _readBGRA8TextureToRGBA8Pixels(t,e,i,r={}){let a=Math.max(1,e|0),n=Math.max(1,i|0),l=4,s=256,f=a*l,p=Math.ceil(f/s)*s,u=this.device?.limits?.maxBufferSize??256*1024*1024,m=Math.max(1024*1024,Math.floor(u*.9)),d=this._getMaxBufferChunkBytes(r.maxBufferChunkBytes);if(d<p&&(d=p),p>m)throw new Error(`_readBGRA8TextureToRGBA8Pixels: bytesPerRow=${p} exceeds safe buffer cap=${m}`);let c=Math.max(1,Math.floor(d/p))|0,v=new Uint8ClampedArray(a*n*4),x=[],b=this.device.createCommandEncoder();for(let y=0;y<n;y+=c){let h=Math.min(c,n-y)|0,P=p*h|0,_=this.device.createBuffer({size:P,usage:GPUBufferUsage.COPY_DST|GPUBufferUsage.MAP_READ});b.copyTextureToBuffer({texture:t,origin:{x:0,y,z:0}},{buffer:_,bytesPerRow:p,rowsPerImage:h},{width:a,height:h,depthOrArrayLayers:1}),x.push({readBuffer:_,y0:y,rows:h})}if(this.queue.submit([b.finish()]),this.queue&&this.queue.onSubmittedWorkDone)try{await this.queue.onSubmittedWorkDone()}catch{}for(let y of x){let{readBuffer:h,y0:P,rows:_}=y;await h.mapAsync(GPUMapMode.READ);let D=h.getMappedRange(),B=new Uint8Array(D);for(let A=0;A<_;A++){let C=A*p,T=(P+A)*a*4;for(let q=0;q<a;q++){let S=C+q*4,R=T+q*4;v[R+0]=B[S+2],v[R+1]=B[S+1],v[R+2]=B[S+0],v[R+3]=B[S+3]}}h.unmap(),h.destroy()}return v}resize(t){this.maxConfigs=t,this._initBuffers()}setPermTable(t){this.queue.writeBuffer(this.permBuffer,0,t)}setPosBuffer(t){this.posBuffer=t}setInputTextureView(t){try{if(((t?.texture?.usage??0)&GPUTextureUsage.TEXTURE_BINDING)===0){console.warn("setInputTextureView: provided texture view not created with TEXTURE_BINDING; ignoring.");return}}catch{}if(this.inputTextureView=t,this._tid!==null){let e=this._texPairs.get(this._tid);e&&(e.bindGroupDirty=!0)}}setOutputTextureView(t){try{if(((t?.texture?.usage??0)&GPUTextureUsage.STORAGE_BINDING)===0){console.warn("setOutputTextureView: provided texture view not created with STORAGE_BINDING; ignoring.");return}}catch{}if(this.outputTextureView=t,this._tid!==null){let e=this._texPairs.get(this._tid);e&&(e.bindGroupDirty=!0)}}buildPermTable(t=Date.now()){let i=new se(t).perm,r=new Uint32Array(512);for(let a=0;a<512;a++)r[a]=i[a];this.setPermTable(r)}setOptions(t={}){Array.isArray(t.noiseChoices)?this.noiseChoices=t.noiseChoices:this.noiseChoices||(this.noiseChoices=[0]);let{getGradient:e=0,outputChannel:i=1,baseRadius:r=0,heightScale:a=1,useCustomPos:n=0,ioFlags:l=0}=t;this.useCustomPos=n>>>0;let s=new ArrayBuffer(32),f=new DataView(s);f.setUint32(0,e,!0),f.setUint32(4,this.useCustomPos,!0),f.setUint32(8,i,!0),f.setUint32(12,l>>>0,!0),f.setFloat32(16,r,!0),f.setFloat32(20,a,!0),f.setFloat32(24,0,!0),f.setFloat32(28,0,!0),this.queue.writeBuffer(this.optionsBuffer,0,s);for(let p of this._texPairs.values())p.bindGroupDirty=!0}setNoiseParams(t={}){let{seed:e=Date.now()|0,zoom:i=1,freq:r=1,octaves:a=8,lacunarity:n=2,gain:l=.5,xShift:s=0,yShift:f=0,zShift:p=0,turbulence:u=0,seedAngle:m=0,exp1:d=1,exp2:c=0,threshold:v=.1,rippleFreq:x=10,time:b=0,warpAmp:y=.5,gaborRadius:h=4,terraceStep:P=8,toroidal:_=0,voroMode:D=0,edgeK:B=0}=t,A=Math.max(Number(i)||0,1e-6),C=Math.max(Number(r)||0,1e-6),T=Math.max(0,Math.floor(Number(a)||0))>>>0,q=(u?1:0)>>>0,S=(_?1:0)>>>0,R=new ArrayBuffer(88),w=new DataView(R),M=0;w.setUint32(M+0,e>>>0,!0),w.setFloat32(M+4,A,!0),w.setFloat32(M+8,C,!0),w.setUint32(M+12,T,!0),w.setFloat32(M+16,n,!0),w.setFloat32(M+20,l,!0),w.setFloat32(M+24,s,!0),w.setFloat32(M+28,f,!0),w.setFloat32(M+32,p,!0),w.setUint32(M+36,q,!0),w.setFloat32(M+40,m,!0),w.setFloat32(M+44,d,!0),w.setFloat32(M+48,c,!0),w.setFloat32(M+52,v,!0),w.setFloat32(M+56,x,!0),w.setFloat32(M+60,b,!0),w.setFloat32(M+64,y,!0),w.setFloat32(M+68,h,!0),w.setFloat32(M+72,P,!0),w.setUint32(M+76,S,!0),w.setUint32(M+80,D>>>0,!0),w.setFloat32(M+84,B,!0),this.queue.writeBuffer(this.paramsBuffer,0,R);for(let L of this._texPairs.values())L.bindGroupDirty=!0;for(let[L,F]of this._volumeCache)!F||!Array.isArray(F.chunks)||(F._bindGroupsDirty=!0)}_compute2DTiling(t,e){let i=Math.min(t,ye),r=Math.min(e,ye),a=Math.ceil(t/i),n=Math.ceil(e/r),l=a*n;return{tileW:i,tileH:r,tilesX:a,tilesY:n,layers:l}}_create2DPair(t,e){let i=this._compute2DTiling(t,e),r=GPUTextureUsage.STORAGE_BINDING|GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_SRC|GPUTextureUsage.COPY_DST,a=()=>this.device.createTexture({size:[i.tileW,i.tileH,i.layers],format:"rgba16float",usage:r}),n={dimension:"2d-array",arrayLayerCount:i.layers},l=a(),s=a(),f=l.createView(n),p=s.createView(n);l.label=`2D texA ${t}x${e}x${i.layers}`,s.label=`2D texB ${t}x${e}x${i.layers}`,f.label="2D:viewA",p.label="2D:viewB",this._tag.set(f,"2D:A"),this._tag.set(p,"2D:B");let u=this._texPairs.size;return this._texPairs.set(u,{texA:l,texB:s,viewA:f,viewB:p,fullWidth:t,fullHeight:e,tileWidth:i.tileW,tileHeight:i.tileH,tilesX:i.tilesX,tilesY:i.tilesY,layers:i.layers,isA:!0,tiles:null,bindGroupDirty:!0}),this._tid===null&&this.setActiveTexture(u),u}createShaderTextures(t,e){this._tid!==null&&this._texPairs.has(this._tid)&&this.destroyTexturePair(this._tid);let i=this._create2DPair(t,e);return this.setActiveTexture(i),i}destroyTexturePair(t){let e=this._texPairs.get(t);if(e){try{e.texA.destroy()}catch{}try{e.texB.destroy()}catch{}if(Array.isArray(e.tiles))for(let i of e.tiles){if(Array.isArray(i.frames))for(let r of i.frames)try{r.destroy()}catch{}if(i.posBuf&&i.posBuf!==this.nullPosBuffer)try{i.posBuf.destroy()}catch{}}this._texPairs.delete(t),this._tid===t&&(this._tid=null,this.inputTextureView=null,this.outputTextureView=null,this.viewA=null,this.viewB=null)}}destroyAllTexturePairs(){let t=Array.from(this._texPairs.keys());for(let e of t)this.destroyTexturePair(e)}setActiveTexture(t){if(!this._texPairs.has(t))throw new Error("setActiveTexture: invalid id");this._tid=t;let e=this._texPairs.get(t);this.viewA=e.viewA,this.viewB=e.viewB,this.width=e.tileWidth,this.height=e.tileHeight,this.layers=e.layers,this.inputTextureView=e.isA?e.viewA:e.viewB,this.outputTextureView=e.isA?e.viewB:e.viewA}_buildPosBuffer(t,e,i){if(!(i instanceof Float32Array)||i.byteLength<=0)return this.nullPosBuffer;let r=Math.max(1,Math.floor(t)),a=Math.max(1,Math.floor(e)),l=r*a*4;if(i.length!==l)throw new Error(`_buildPosBuffer: customData length ${i.length} != expected ${l} (width=${r}, height=${a})`);let s=this.device?.limits?.maxBufferSize??2147483648,f=Math.floor(s*.98);if(i.byteLength>f)throw new Error(`_buildPosBuffer: ${i.byteLength} bytes exceeds maxBufferSize ${s} (w=${r}, h=${a})`);let p=this.device.createBuffer({size:i.byteLength,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST});return this._writeBufferChunked(p,0,i.buffer,i.byteOffset,i.byteLength,this.maxBufferChunkBytes),p}_writeFrameUniform(t,e){let i=new ArrayBuffer(64),r=new DataView(i);r.setUint32(0,e.fullWidth>>>0,!0),r.setUint32(4,e.fullHeight>>>0,!0),r.setUint32(8,e.tileWidth>>>0,!0),r.setUint32(12,e.tileHeight>>>0,!0),r.setInt32(16,e.originX|0,!0),r.setInt32(20,e.originY|0,!0),r.setInt32(24,e.originZ|0,!0),r.setUint32(28,e.fullDepth>>>0,!0),r.setUint32(32,e.tileDepth>>>0,!0),r.setInt32(36,e.layerIndex|0,!0),r.setUint32(40,e.layers>>>0,!0),r.setUint32(44,0,!0),r.setFloat32(48,e.originXf??0,!0),r.setFloat32(52,e.originYf??0,!0),r.setFloat32(56,0,!0),r.setFloat32(60,0,!0),this.queue.writeBuffer(t,0,i)}_create2DTileBindGroups(t,e={}){let i=this._texPairs.get(t);if(!i)throw new Error("_create2DTileBindGroups: invalid tid");let a=((e.useCustomPos??0)|0)!==0&&e.customData instanceof Float32Array?e.customData:null,n=!!a,l=Array.isArray(i.tiles)&&i.tiles.some(f=>f&&f.posIsCustom);if(!n&&l&&(i.bindGroupDirty=!0),Array.isArray(i.tiles)&&!i.bindGroupDirty&&!n)return;let s=[];for(let f=0;f<i.tilesY;f++)for(let p=0;p<i.tilesX;p++){let u=f*i.tilesX+p,m=p*i.tileWidth,d=f*i.tileHeight,c=i.tiles&&i.tiles[u]||null,v=this.nullPosBuffer,x=!1;if(n)v=this._buildPosBuffer(i.tileWidth,i.tileHeight,a),x=v!==this.nullPosBuffer;else if(c&&c.posBuf&&!c.posIsCustom)v=c.posBuf,x=!1;else if(v=this.nullPosBuffer,x=!1,c&&c.posBuf&&c.posIsCustom)try{c.posBuf.destroy()}catch{}let b;c&&c.frames&&c.frames[0]?b=c.frames[0]:b=this.device.createBuffer({size:64,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});let y=Number.isFinite(e.frameFullWidth)?e.frameFullWidth>>>0:i.fullWidth,h=Number.isFinite(e.frameFullHeight)?e.frameFullHeight>>>0:i.fullHeight,P=e.squareWorld||String(e.worldMode||"").toLowerCase()==="crop";if(e.squareWorld){let C=Math.max(y,h,i.fullWidth,i.fullHeight)>>>0;y=C,h=C}let _,D;if(P)_=m,D=d;else{let C=y/i.fullWidth,T=h/i.fullHeight;_=m*C,D=d*T}this._writeFrameUniform(b,{fullWidth:y,fullHeight:h,tileWidth:i.tileWidth,tileHeight:i.tileHeight,originX:m,originY:d,originZ:0,fullDepth:1,tileDepth:1,layerIndex:u,layers:i.layers,originXf:_,originYf:D});let B=c?.bgs?.[0]?.bgA??null,A=c?.bgs?.[0]?.bgB??null;(!B||!A||i.bindGroupDirty)&&(B=this.device.createBindGroup({layout:this.bindGroupLayout,entries:[{binding:0,resource:{buffer:this.optionsBuffer}},{binding:1,resource:{buffer:this.paramsBuffer}},{binding:2,resource:{buffer:this.permBuffer}},{binding:3,resource:i.viewA},{binding:4,resource:i.viewB},{binding:5,resource:{buffer:v}},{binding:6,resource:{buffer:b}},{binding:7,resource:this._dummy3D_sampleView},{binding:8,resource:this._dummy3D_writeView}]}),A=this.device.createBindGroup({layout:this.bindGroupLayout,entries:[{binding:0,resource:{buffer:this.optionsBuffer}},{binding:1,resource:{buffer:this.paramsBuffer}},{binding:2,resource:{buffer:this.permBuffer}},{binding:3,resource:i.viewB},{binding:4,resource:i.viewA},{binding:5,resource:{buffer:v}},{binding:6,resource:{buffer:b}},{binding:7,resource:this._dummy3D_sampleView},{binding:8,resource:this._dummy3D_writeView}]})),s.push({layerIndex:u,originX:m,originY:d,frames:[b],posBuf:v,posIsCustom:x,bgs:[{bgA:B,bgB:A}]})}i.tiles=s,i.bindGroupDirty=!1,this._tid===t&&(this._tiles=s)}async _runPipelines(t,e,i,r,a,n,l=1){let s=t,f=e,p=Array.isArray(n),u=0,m=this.device.createCommandEncoder(),d=m.beginComputePass();for(let c of this.noiseChoices){let v=typeof c=="number"?this.entryPoints[c]:c,x=this.pipelines.get(v);x||(x=this.device.createComputePipeline({layout:this.pipelineLayout,compute:{module:this.shaderModule,entryPoint:v}}),this.pipelines.set(v,x)),p&&this.setNoiseParams(n[u++]),d.setPipeline(x),d.setBindGroup(0,s),d.dispatchWorkgroups(Math.ceil(i/8),Math.ceil(r/8),l),[s,f]=[f,s]}return d.end(),this.queue.submit([m.finish()]),f}async computeToTexture(t,e,i={},r={}){let a=t|0,n=e|0;if(!(a>0&&n>0))throw new Error(`computeToTexture: invalid size ${t}x${e}`);if(this._tid!=null){let b=this._texPairs.get(this._tid);(!b||b.fullWidth!==a||b.fullHeight!==n)&&this.destroyTexturePair(this._tid)}if(this._tid==null){let b=this._create2DPair(a,n);this.setActiveTexture(b)}let l=this._texPairs.get(this._tid);if(!l)throw new Error("computeToTexture: failed to create 2D texture pair");i&&!Array.isArray(i)&&this.setNoiseParams(i);let s=r||{},p=((s.useCustomPos??0)|0)!==0&&s.customData instanceof Float32Array?s.customData:null,u=p?1:0;this.setOptions({...s,ioFlags:0,useCustomPos:u});let m={...s,useCustomPos:u,customData:p};(!l.tiles||l.bindGroupDirty||p)&&this._create2DTileBindGroups(this._tid,m);let d=l.isA,c=null,v=null;for(let b of l.tiles){let{bgA:y,bgB:h}=b.bgs[0],P=c?c===y?y:h:d?y:h,_=P===y?h:y;c=await this._runPipelines(P,_,l.tileWidth,l.tileHeight,1,i,1),v={bgA:y,bgB:h}}let x=c===v.bgB;return l.isA=x,this.isA=x,this.setActiveTexture(this._tid),this.getCurrentView()}getCurrentView(){let t=this._texPairs.get(this._tid);return t?t.isA?t.viewA:t.viewB:null}_compute3DTiling(t,e,i){let r=Math.min(t,ne),a=Math.min(e,ne),n=this.device?.limits?.maxBufferSize??256*1024*1024,l=r*a*Re,s=Math.max(1,Math.floor(n*.8/Math.max(1,l))),f=Math.min(i,ne,s),p=Math.ceil(t/r),u=Math.ceil(e/a),m=Math.ceil(i/f);return{tw:r,th:a,td:f,nx:p,ny:u,nz:m}}_create3DChunks(t,e,i){let r=this._compute3DTiling(t,e,i),a=[],n=GPUTextureUsage.STORAGE_BINDING|GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_SRC|GPUTextureUsage.COPY_DST;for(let l=0;l<r.nz;l++)for(let s=0;s<r.ny;s++)for(let f=0;f<r.nx;f++){let p=f*r.tw,u=s*r.th,m=l*r.td,d=this.device.createTexture({size:{width:r.tw,height:r.th,depthOrArrayLayers:r.td},dimension:"3d",format:"rgba16float",usage:n}),c=this.device.createTexture({size:{width:r.tw,height:r.th,depthOrArrayLayers:r.td},dimension:"3d",format:"rgba16float",usage:n}),v=d.createView({dimension:"3d"}),x=c.createView({dimension:"3d"});d.label=`3D texA ${r.tw}x${r.th}x${r.td} @ (${f},${s},${l})`,c.label=`3D texB ${r.tw}x${r.th}x${r.td} @ (${f},${s},${l})`,v.label=`3D:viewA[${f},${s},${l}]`,x.label=`3D:viewB[${f},${s},${l}]`,this._tag.set(v,`3D:A[${f},${s},${l}]`),this._tag.set(x,`3D:B[${f},${s},${l}]`),a.push({texA:d,texB:c,viewA:v,viewB:x,ox:p,oy:u,oz:m,w:r.tw,h:r.th,d:r.td,isA:!0,fb:null,posBuf:null,bgA:null,bgB:null})}return{chunks:a,tile:{w:r.tw,h:r.th,d:r.td},full:{w:t,h:e,d:i},grid:{nx:r.nx,ny:r.ny,nz:r.nz}}}_destroy3DSet(t){if(t)for(let e of t.chunks){try{e.texA.destroy()}catch{}try{e.texB.destroy()}catch{}if(e.viewA=null,e.viewB=null,e.bgA=null,e.bgB=null,e.fb){try{e.fb.destroy()}catch{}e.fb=null}if(e.posBuf&&e.posBuf!==this.nullPosBuffer){try{e.posBuf.destroy()}catch{}e.posBuf=null}}}destroyAllVolumes(){for(let[t,e]of this._volumeCache)this._destroy3DSet(e),this._volumeCache.delete(t)}get3DView(t){let e=this._volumeCache.get(String(t));if(!e)return null;let i=e.chunks.map(r=>r.isA?r.viewA:r.viewB);return i.length===1?i[0]:{views:i,meta:{full:e.full,tile:e.tile,grid:e.grid}}}destroyVolume(t){let e=String(t),i=this._volumeCache.get(e);i&&(this._destroy3DSet(i),this._volumeCache.delete(e))}_getOrCreate3DVolume(t,e,i,r=null,a=null){let n=r?String(r):`${t}x${e}x${i}`,l=this._volumeCache.get(n);if(l)return l;l=this._create3DChunks(t,e,i);for(let s of l.chunks){s.fb=this.device.createBuffer({size:64,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});let f=a&&Number.isFinite(a?.w)?a.w>>>0:l.full.w,p=a&&Number.isFinite(a?.h)?a.h>>>0:l.full.h,u=a&&Number.isFinite(a?.d)?a.d>>>0:l.full.d,m=f/l.full.w,d=p/l.full.h,c=s.ox*m,v=s.oy*d;this._writeFrameUniform(s.fb,{fullWidth:f,fullHeight:p,tileWidth:s.w,tileHeight:s.h,originX:s.ox,originY:s.oy,originZ:s.oz,fullDepth:u,tileDepth:s.d,layerIndex:0,layers:1,originXf:c,originYf:v});let x=this._buildPosBuffer(s.w,s.h,null);s.posBuf=x;try{s.bgA=this.device.createBindGroup({layout:this.bindGroupLayout,entries:[{binding:0,resource:{buffer:this.optionsBuffer}},{binding:1,resource:{buffer:this.paramsBuffer}},{binding:2,resource:{buffer:this.permBuffer}},{binding:3,resource:this._dummy2D_sampleView},{binding:4,resource:this._dummy2D_writeView},{binding:5,resource:{buffer:x}},{binding:6,resource:{buffer:s.fb}},{binding:7,resource:s.viewA},{binding:8,resource:s.viewB}]}),s.bgB=this.device.createBindGroup({layout:this.bindGroupLayout,entries:[{binding:0,resource:{buffer:this.optionsBuffer}},{binding:1,resource:{buffer:this.paramsBuffer}},{binding:2,resource:{buffer:this.permBuffer}},{binding:3,resource:this._dummy2D_sampleView},{binding:4,resource:this._dummy2D_writeView},{binding:5,resource:{buffer:s.posBuf}},{binding:6,resource:{buffer:s.fb}},{binding:7,resource:s.viewB},{binding:8,resource:s.viewA}]})}catch(b){throw new Error(`_getOrCreate3DVolume: createBindGroup failed: ${b?.message||b}`)}}return l._bindGroupsDirty=!1,this._volumeCache.set(n,l),l}_recreate3DBindGroups(t,e=null){if(!t||!Array.isArray(t.chunks))return;let i=e&&Number.isFinite(e.w)?e.w>>>0:t.full.w,r=e&&Number.isFinite(e.h)?e.h>>>0:t.full.h,a=e&&Number.isFinite(e.d)?e.d>>>0:t.full.d;for(let n of t.chunks){if(!n.fb){n.fb=this.device.createBuffer({size:64,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});let f=i/t.full.w,p=r/t.full.h,u=n.ox*f,m=n.oy*p;this._writeFrameUniform(n.fb,{fullWidth:i,fullHeight:r,tileWidth:n.w,tileHeight:n.h,originX:n.ox,originY:n.oy,originZ:n.oz,fullDepth:a,tileDepth:n.d,layerIndex:0,layers:1,originXf:u,originYf:m})}n.posBuf||(n.posBuf=this._buildPosBuffer(n.w,n.h,null));let l=[{binding:0,resource:{buffer:this.optionsBuffer}},{binding:1,resource:{buffer:this.paramsBuffer}},{binding:2,resource:{buffer:this.permBuffer}},{binding:3,resource:this._dummy2D_sampleView},{binding:4,resource:this._dummy2D_writeView},{binding:5,resource:{buffer:n.posBuf}},{binding:6,resource:{buffer:n.fb}},{binding:7,resource:n.viewA},{binding:8,resource:n.viewB}],s=[{binding:0,resource:{buffer:this.optionsBuffer}},{binding:1,resource:{buffer:this.paramsBuffer}},{binding:2,resource:{buffer:this.permBuffer}},{binding:3,resource:this._dummy2D_sampleView},{binding:4,resource:this._dummy2D_writeView},{binding:5,resource:{buffer:n.posBuf}},{binding:6,resource:{buffer:n.fb}},{binding:7,resource:n.viewB},{binding:8,resource:n.viewA}];try{n.bgA=this.device.createBindGroup({layout:this.bindGroupLayout,entries:l}),n.bgB=this.device.createBindGroup({layout:this.bindGroupLayout,entries:s})}catch(f){throw new Error(`_recreate3DBindGroups: failed to create bind groups: ${f?.message||f}`)}}t._bindGroupsDirty=!1}async computeToTexture3D(t,e,i,r={},a={}){let n=t|0,l=e|0,s=i|0;if(!(n>0&&l>0&&s>0))throw new Error(`computeToTexture3D: invalid size ${t}x${e}x${i}`);r&&!Array.isArray(r)&&this.setNoiseParams(r);let f=a||{};this.setOptions({...f,ioFlags:3,useCustomPos:f.useCustomPos??this.useCustomPos});let p=a&&(Number.isFinite(a.frameFullWidth)||Number.isFinite(a.frameFullHeight)||Number.isFinite(a.frameFullDepth))?{w:Number.isFinite(a.frameFullWidth)?a.frameFullWidth>>>0:n,h:Number.isFinite(a.frameFullHeight)?a.frameFullHeight>>>0:l,d:Number.isFinite(a.frameFullDepth)?a.frameFullDepth>>>0:s}:null,u=this._getOrCreate3DVolume(n,l,s,a.id,p);if(!u)throw new Error("computeToTexture3D: failed to create or retrieve volume");(u._bindGroupsDirty||!u.chunks[0].bgA||!u.chunks[0].bgB)&&this._recreate3DBindGroups(u,p);let m=null;for(let c of u.chunks){let v=c.isA?c.bgA:c.bgB,x=c.isA?c.bgB:c.bgA;if(!v||!x)throw new Error("computeToTexture3D: missing bind groups (volume not initialized correctly)");m=await this._runPipelines(v,x,c.w,c.h,c.d,r,c.d),c.isA=m===c.bgB}let d=u.chunks.map(c=>c.isA?c.viewA:c.viewB);return d.length===1?d[0]:{views:d,meta:{full:u.full,tile:u.tile,grid:u.grid}}}configureCanvas(t){let e=navigator.gpu.getPreferredCanvasFormat&&navigator.gpu.getPreferredCanvasFormat()||"bgra8unorm",i=t.getContext("webgpu");i.configure({device:this.device,format:e,alphaMode:"opaque",size:[t.width,t.height]}),this._ctxMap.set(t,{ctx:i,size:[t.width,t.height]})}initBlitRender(){this.sampler||(this.sampler=this.device.createSampler({magFilter:"linear",minFilter:"linear",addressModeU:"clamp-to-edge",addressModeV:"clamp-to-edge"})),this.bgl2D||(this.bgl2D=this.device.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.FRAGMENT,sampler:{}},{binding:1,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"float",viewDimension:"2d-array"}},{binding:2,visibility:GPUShaderStage.FRAGMENT,buffer:{type:"uniform"}}]}),this.pipeline2D=this.device.createRenderPipeline({layout:this.device.createPipelineLayout({bindGroupLayouts:[this.bgl2D]}),vertex:{module:this.device.createShaderModule({code:ae}),entryPoint:"vs_main"},fragment:{module:this.device.createShaderModule({code:ae}),entryPoint:"fs_main",targets:[{format:"bgra8unorm"}]},primitive:{topology:"triangle-list"}}),this.blit2DUbo=this.device.createBuffer({size:16,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST})),this.bgl3D||(this.bgl3D=this.device.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.FRAGMENT,sampler:{}},{binding:1,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"float",viewDimension:"3d"}},{binding:2,visibility:GPUShaderStage.FRAGMENT,buffer:{type:"uniform"}}]}),this.pipeline3D=this.device.createRenderPipeline({layout:this.device.createPipelineLayout({bindGroupLayouts:[this.bgl3D]}),vertex:{module:this.device.createShaderModule({code:oe}),entryPoint:"vs_main"},fragment:{module:this.device.createShaderModule({code:oe}),entryPoint:"fs_main",targets:[{format:"bgra8unorm"}]},primitive:{topology:"triangle-list"}}),this.blit3DUbo=this.device.createBuffer({size:16,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}))}_renderCommonCanvasSetup(t,e){let i="bgra8unorm",r=this._ctxMap.get(t);if(r){let l=t.width|0,s=t.height|0;(r.size[0]!==l||r.size[1]!==s)&&(r.size=[l,s],r.ctx.configure({device:this.device,format:i,alphaMode:"opaque",size:r.size}))}else{let l=t.getContext("webgpu"),s=[t.width|0,t.height|0];l.configure({device:this.device,format:i,alphaMode:"opaque",size:s}),r={ctx:l,size:s},this._ctxMap.set(t,r)}let a=this.device.createCommandEncoder(),n=a.beginRenderPass({colorAttachments:[{view:r.ctx.getCurrentTexture().createView(),loadOp:e?"clear":"load",clearValue:{r:0,g:0,b:0,a:1},storeOp:"store"}]});return{enc:a,pass:n,ctxEntry:r}}renderTextureToCanvas(t,e,i={}){let{layer:r=0,channel:a=0,preserveCanvasSize:n=!0,clear:l=!0}=i;if(this.initBlitRender(),!n)try{let m=t.texture;m&&typeof m.width=="number"&&typeof m.height=="number"&&(e.width=m.width,e.height=m.height)}catch{}let s=new Uint32Array([r>>>0,a>>>0,0,0]);this.queue.writeBuffer(this.blit2DUbo,0,s.buffer,s.byteOffset,s.byteLength);let f=this.device.createBindGroup({layout:this.bgl2D,entries:[{binding:0,resource:this.sampler},{binding:1,resource:t},{binding:2,resource:{buffer:this.blit2DUbo}}]}),{enc:p,pass:u}=this._renderCommonCanvasSetup(e,l);u.setPipeline(this.pipeline2D),u.setBindGroup(0,f),u.draw(6,1,0,0),u.end(),this.queue.submit([p.finish()])}renderTexture3DSliceToCanvas(t,e,i={}){let{depth:r,slice:a=0,zNorm:n=null,channel:l=0,chunk:s=0,preserveCanvasSize:f=!0,clear:p=!0}=i;this.initBlitRender();let u,m;if(t&&t.views&&Array.isArray(t.views)?(u=t.views[Math.max(0,Math.min(s|0,t.views.length-1))],m=t.meta?.tile?.d??r):(u=t,m=r),!u||!m)throw new Error("renderTexture3DSliceToCanvas: need a 3D view and its depth");if(!f)try{let h=u.texture;h&&typeof h.width=="number"&&typeof h.height=="number"&&(e.width=h.width,e.height=h.height)}catch{}let d=n??(Math.min(Math.max(a,0),m-1)+.5)/m;d=Math.min(Math.max(d,0),1);let c=new ArrayBuffer(16),v=new DataView(c);v.setFloat32(0,d,!0),v.setUint32(4,l>>>0,!0),v.setUint32(8,0,!0),v.setUint32(12,0,!0),this.queue.writeBuffer(this.blit3DUbo,0,c);let x=this.device.createBindGroup({layout:this.bgl3D,entries:[{binding:0,resource:this.sampler},{binding:1,resource:u},{binding:2,resource:{buffer:this.blit3DUbo}}]}),{enc:b,pass:y}=this._renderCommonCanvasSetup(e,p);y.setPipeline(this.pipeline3D),y.setBindGroup(0,x),y.draw(6,1,0,0),y.end(),this.queue.submit([b.finish()])}setExportBackground(t="black"){this.exportBackground=t}_resolveExportBackground(t){let e=t===void 0?this.exportBackground:t;if(e==null)return{r:0,g:0,b:0,a:1,transparent:!1};if(typeof e=="string"){let r=e.trim().toLowerCase();if(r==="transparent")return{r:0,g:0,b:0,a:0,transparent:!0};if(r==="black")return{r:0,g:0,b:0,a:1,transparent:!1};if(r==="white")return{r:1,g:1,b:1,a:1,transparent:!1};if(r[0]==="#")return this._parseHexBackground(r)}let i=r=>{let a=Number(r);if(!Number.isFinite(a))return 0;let n=a>1?a/255:a;return Math.min(Math.max(n,0),1)};if(Array.isArray(e)){let r=i(e[0]),a=i(e[1]),n=i(e[2]),l=e.length>=4?i(e[3]):1;return{r,g:a,b:n,a:l,transparent:l<=0}}if(typeof e=="object"){let r=i(e.r),a=i(e.g),n=i(e.b),l=e.a===void 0?1:i(e.a);return{r,g:a,b:n,a:l,transparent:l<=0}}return{r:0,g:0,b:0,a:1,transparent:!1}}_parseHexBackground(t){let e=String(t).trim().replace(/^#/,""),i=m=>m+m,r=0,a=0,n=0,l=255;if(e.length===3||e.length===4)r=parseInt(i(e[0]),16),a=parseInt(i(e[1]),16),n=parseInt(i(e[2]),16),e.length===4&&(l=parseInt(i(e[3]),16));else if(e.length===6||e.length===8)r=parseInt(e.slice(0,2),16),a=parseInt(e.slice(2,4),16),n=parseInt(e.slice(4,6),16),e.length===8&&(l=parseInt(e.slice(6,8),16));else return{r:0,g:0,b:0,a:1,transparent:!1};let s=r/255,f=a/255,p=n/255,u=l/255;return{r:s,g:f,b:p,a:u,transparent:u<=0}}_applyExportBackground(t,e){if(!t||!e||e.transparent)return;let i=Math.round(e.r*255),r=Math.round(e.g*255),a=Math.round(e.b*255),n=Math.round((e.a??1)*255);if(n<=0)return;let l=t.length|0;if(n>=255){for(let s=0;s<l;s+=4){let f=t[s+3]|0;if(f===255)continue;if(f===0){t[s+0]=i,t[s+1]=r,t[s+2]=a,t[s+3]=255;continue}let p=255-f;t[s+0]=(t[s+0]*f+i*p)/255|0,t[s+1]=(t[s+1]*f+r*p)/255|0,t[s+2]=(t[s+2]*f+a*p)/255|0,t[s+3]=255}return}for(let s=0;s<l;s+=4){let f=t[s+0]|0,p=t[s+1]|0,u=t[s+2]|0,m=t[s+3]|0,d=m+n*(255-m)/255|0;if(d<=0){t[s+0]=0,t[s+1]=0,t[s+2]=0,t[s+3]=0;continue}let c=i*n|0,v=r*n|0,x=a*n|0,b=f*m|0,y=p*m|0,h=u*m|0,P=255-m|0,_=b+c*P/255|0,D=y+v*P/255|0,B=h+x*P/255|0;t[s+0]=Math.min(255,Math.max(0,_*255/d|0)),t[s+1]=Math.min(255,Math.max(0,D*255/d|0)),t[s+2]=Math.min(255,Math.max(0,B*255/d|0)),t[s+3]=Math.min(255,Math.max(0,d))}}_forceOpaqueAlpha(t){let e=t.length|0;for(let i=3;i<e;i+=4)t[i]=255}async export2DTextureToPNGBlob(t,e,i,r={}){if(!t)throw new Error("export2DTextureToPNGBlob: textureView is required");let a=Math.max(1,e|0),n=Math.max(1,i|0),l=r.layer??0,s=r.channel??0,f=this._resolveExportBackground(r.background);if(this.initBlitRender(),this.queue&&this.queue.onSubmittedWorkDone)try{await this.queue.onSubmittedWorkDone()}catch{}let u=this.device.createTexture({size:[a,n,1],format:"bgra8unorm",usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.COPY_SRC}),m=new Uint32Array([l>>>0,s>>>0,0,0]);this.queue.writeBuffer(this.blit2DUbo,0,m.buffer,m.byteOffset,m.byteLength);let d=this.device.createBindGroup({layout:this.bgl2D,entries:[{binding:0,resource:this.sampler},{binding:1,resource:t},{binding:2,resource:{buffer:this.blit2DUbo}}]}),c=this.device.createCommandEncoder(),v=c.beginRenderPass({colorAttachments:[{view:u.createView(),loadOp:"clear",storeOp:"store",clearValue:{r:0,g:0,b:0,a:0}}]});if(v.setPipeline(this.pipeline2D),v.setBindGroup(0,d),v.draw(6,1,0,0),v.end(),this.queue.submit([c.finish()]),this.queue&&this.queue.onSubmittedWorkDone)try{await this.queue.onSubmittedWorkDone()}catch{}let x=await this._readBGRA8TextureToRGBA8Pixels(u,a,n,{maxBufferChunkBytes:r.maxBufferChunkBytes??this.maxBufferChunkBytes});u.destroy();let b=r.useAlphaForBackground===!0;f.transparent||b?this._applyExportBackground(x,f):this._forceOpaqueAlpha(x);let y=document.createElement("canvas");y.width=a,y.height=n;let h=y.getContext("2d");if(!h)throw new Error("export2DTextureToPNGBlob: unable to get 2D context");return h.putImageData(new ImageData(x,a,n),0,0),await new Promise((_,D)=>{y.toBlob(B=>{B?_(B):D(new Error("export2DTextureToPNGBlob: toBlob returned null"))},"image/png")})}async exportCurrent2DToPNGBlob(t,e,i={}){let r=this.getCurrentView();if(!r)throw new Error("exportCurrent2DToPNGBlob: no active 2D texture view");return this.export2DTextureToPNGBlob(r,t,e,i)}async export3DSliceToPNGBlob(t,e,i,r={}){if(!t)throw new Error("export3DSliceToPNGBlob: target is required");let a=Math.max(1,e|0),n=Math.max(1,i|0),{depth:l,slice:s=0,zNorm:f=null,channel:p=0,chunk:u=0}=r;if(!l||l<=0)throw new Error("export3DSliceToPNGBlob: depth must be provided and > 0");let m=this._resolveExportBackground(r.background);if(this.initBlitRender(),this.queue&&this.queue.onSubmittedWorkDone)try{await this.queue.onSubmittedWorkDone()}catch{}let d,c;if(t&&t.views&&Array.isArray(t.views)){let k=Math.max(0,Math.min(u|0,t.views.length-1));d=t.views[k],c=t.meta?.tile?.d??l}else d=t,c=l;if(!d||!c)throw new Error("export3DSliceToPNGBlob: need a 3D view and its depth");let v=f??(Math.min(Math.max(s,0),c-1)+.5)/c;v=Math.min(Math.max(v,0),1);let b=this.device.createTexture({size:[a,n,1],format:"bgra8unorm",usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.COPY_SRC}),y=new ArrayBuffer(16),h=new DataView(y);h.setFloat32(0,v,!0),h.setUint32(4,p>>>0,!0),h.setUint32(8,0,!0),h.setUint32(12,0,!0),this.queue.writeBuffer(this.blit3DUbo,0,y);let P=this.device.createBindGroup({layout:this.bgl3D,entries:[{binding:0,resource:this.sampler},{binding:1,resource:d},{binding:2,resource:{buffer:this.blit3DUbo}}]}),_=this.device.createCommandEncoder(),D=_.beginRenderPass({colorAttachments:[{view:b.createView(),loadOp:"clear",storeOp:"store",clearValue:{r:0,g:0,b:0,a:0}}]});D.setPipeline(this.pipeline3D),D.setBindGroup(0,P),D.draw(6,1,0,0),D.end();let B=4,A=256,C=a*B,T=Math.ceil(C/A)*A,q=T*n,S=this.device.createBuffer({size:q,usage:GPUBufferUsage.COPY_DST|GPUBufferUsage.MAP_READ});_.copyTextureToBuffer({texture:b},{buffer:S,bytesPerRow:T,rowsPerImage:n},{width:a,height:n,depthOrArrayLayers:1}),this.queue.submit([_.finish()]),this.queue&&this.queue.onSubmittedWorkDone&&await this.queue.onSubmittedWorkDone(),await S.mapAsync(GPUMapMode.READ);let R=S.getMappedRange(),w=new Uint8Array(R),M=new Uint8ClampedArray(a*n*B),L=0;for(let k=0;k<n;k++){let g=k*T;for(let z=0;z<a;z++){let E=g+z*4;M[L++]=w[E+2],M[L++]=w[E+1],M[L++]=w[E+0],M[L++]=w[E+3]}}S.unmap(),S.destroy(),b.destroy(),this._applyExportBackground(M,m);let F=document.createElement("canvas");F.width=a,F.height=n;let W=F.getContext("2d");if(!W)throw new Error("export3DSliceToPNGBlob: unable to get 2D context");return W.putImageData(new ImageData(M,a,n),0,0),await new Promise((k,g)=>{F.toBlob(z=>{z?k(z):g(new Error("export3DSliceToPNGBlob: toBlob returned null"))},"image/png")})}async _render3DSliceToRGBA8Pixels(t,e,i,r,a=0,n=null){if(!t)throw new Error("_render3DSliceToRGBA8Pixels: view3D is required");let l=Math.max(1,e|0),s=Math.max(1,i|0);this.initBlitRender();let f=Math.min(Math.max(Number(r)||0,0),1),u=this.device.createTexture({size:[l,s,1],format:"bgra8unorm",usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.COPY_SRC}),m=new ArrayBuffer(16),d=new DataView(m);d.setFloat32(0,f,!0),d.setUint32(4,a>>>0,!0),d.setUint32(8,0,!0),d.setUint32(12,0,!0),this.queue.writeBuffer(this.blit3DUbo,0,m);let c=this.device.createBindGroup({layout:this.bgl3D,entries:[{binding:0,resource:this.sampler},{binding:1,resource:t},{binding:2,resource:{buffer:this.blit3DUbo}}]}),v=this.device.createCommandEncoder(),x=v.beginRenderPass({colorAttachments:[{view:u.createView(),loadOp:"clear",storeOp:"store",clearValue:{r:0,g:0,b:0,a:0}}]});x.setPipeline(this.pipeline3D),x.setBindGroup(0,c),x.draw(6,1,0,0),x.end();let b=4,y=256,h=l*b,P=Math.ceil(h/y)*y,_=P*s,D=this.device.createBuffer({size:_,usage:GPUBufferUsage.COPY_DST|GPUBufferUsage.MAP_READ});v.copyTextureToBuffer({texture:u},{buffer:D,bytesPerRow:P,rowsPerImage:s},{width:l,height:s,depthOrArrayLayers:1}),this.queue.submit([v.finish()]),this.queue&&this.queue.onSubmittedWorkDone&&await this.queue.onSubmittedWorkDone(),await D.mapAsync(GPUMapMode.READ);let B=D.getMappedRange(),A=new Uint8Array(B),C=new Uint8ClampedArray(l*s*b),T=0;for(let q=0;q<s;q++){let S=q*P;for(let R=0;R<l;R++){let w=S+R*4;C[T++]=A[w+2],C[T++]=A[w+1],C[T++]=A[w+0],C[T++]=A[w+3]}}return D.unmap(),D.destroy(),u.destroy(),n&&this._applyExportBackground(C,n),C}async export3DTilesetToPNGBlob(t,e,i,r={}){if(!t)throw new Error("export3DTilesetToPNGBlob: target is required");let a=Math.max(1,e|0),n=Math.max(1,(i??e)|0),{depth:l,channel:s=0,chunk:f=0,tilesAcross:p=16,tilesDown:u=null,startSlice:m=0,sliceCount:d=null}=r,c=this._resolveExportBackground(r.background);if(this.initBlitRender(),this.queue&&this.queue.onSubmittedWorkDone)try{await this.queue.onSubmittedWorkDone()}catch{}let v,x;if(t&&t.views&&Array.isArray(t.views)){let S=Math.max(0,Math.min(f|0,t.views.length-1));v=t.views[S],x=t.meta?.tile?.d??l}else v=t,x=l;if(!v)throw new Error("export3DTilesetToPNGBlob: missing 3D view");if(!x||x<=0)throw new Error("export3DTilesetToPNGBlob: depth must be provided and > 0");let b=Math.max(1,p|0),y=u!=null?Math.max(1,u|0):Math.ceil(x/b),h=Math.min(Math.max(m|0,0),x-1),P=d!=null?Math.max(0,d|0):x-h,_=a*b,D=n*y,B=new Uint8ClampedArray(_*D*4),A=Math.min(x,h+P);for(let S=h;S<A;S++){let R=S-h,w=R%b,M=R/b|0;if(M>=y)break;let L=(S+.5)/x,F=await this._render3DSliceToRGBA8Pixels(v,a,n,L,s,c),W=w*a,Q=M*n;for(let k=0;k<n;k++){let g=k*a*4,z=((Q+k)*_+W)*4;B.set(F.subarray(g,g+a*4),z)}}let C=document.createElement("canvas");C.width=_,C.height=D;let T=C.getContext("2d");if(!T)throw new Error("export3DTilesetToPNGBlob: unable to get 2D context");return T.putImageData(new ImageData(B,_,D),0,0),await new Promise((S,R)=>{C.toBlob(w=>{w?S(w):R(new Error("export3DTilesetToPNGBlob: toBlob returned null"))},"image/png")})}_installAllocDebug(t=256*1024*1024){if(this._allocDebugInstalled)return;this._allocDebugInstalled=!0;let e=this.device,i=e.createBuffer.bind(e);e.createBuffer=a=>{let n=Number(a?.size??0);return Number.isFinite(n)&&n>=t&&console.warn("[alloc] createBuffer",{size:n,usage:a?.usage,mappedAtCreation:a?.mappedAtCreation},`
`,new Error().stack),i(a)};let r=e.createTexture.bind(e);e.createTexture=a=>{let n=r(a);try{let l=a?.format,s=l==="rgba16float"?8:l==="rgba8unorm"||l==="bgra8unorm"?4:0,f=a?.dimension||"2d",p=a?.size,u=0,m=0,d=1;if(Array.isArray(p)?(u=p[0]|0,m=p[1]|0,d=(p[2]??1)|0):p&&typeof p=="object"&&(u=p.width|0,m=p.height|0,d=(p.depthOrArrayLayers??1)|0),s&&u>0&&m>0&&d>0){let c=u*m*d*s;c>=t&&console.warn("[alloc] createTexture",{format:l,dimension:f,size:a?.size,approxBytes:c,usage:a?.usage},`
`,new Error().stack)}}catch{}return n}}},se=class{constructor(t=Date.now()){t<1e7&&(t*=1e7),this.seedN=t,this.seedK=t,this.perm=new Uint8Array(512),this.seed(t)}seed(t){let e=this.xorshift(t);for(let i=0;i<256;i++)this.perm[i]=i;for(let i=255;i>0;i--){let r=Math.floor(e()*(i+1));[this.perm[i],this.perm[r]]=[this.perm[r],this.perm[i]]}for(let i=0;i<256;i++)this.perm[i+256]=this.perm[i]}setSeed(t){this.seedN=t,this.seed(t),this.resetSeed()}random(t,e,i){let r;return typeof i=="number"?r=this.perm[(t&255)+this.perm[(e&255)+this.perm[i&255]]]&255:r=this.perm[(t&255)+this.perm[e&255]]&255,this.perm[r]/255*2-1}seededRandom(){this.seedK+=Math.E;let t=1e9*Math.sin(this.seedK);return t-Math.floor(t)}resetSeed(){this.seedK=this.seedN}xorshift(t){let e=t;return function(){return e^=e<<13,e^=e>>17,e^=e<<5,(e<0?1+~e:e)/4294967295}}dot(t,e=0,i=0,r=0){return t[0]*e+t[1]*i+t[2]*r}};document.body.insertAdjacentHTML("afterbegin",xe);var ke=6,Ve={computeCellular:"CellularPattern",computeWorley:"WorleyPattern",computeAntiCellular:"AntiCellularPattern",computeAntiWorley:"AntiWorleyPattern",computeWhiteNoise:"White Noise",computeBlueNoise:"Blue Noise"},Ie={computeRidge:{clamp:{freq:[.25,8],gain:[.2,.8],octaves:[1,12]}},computeAntiRidge:{clamp:{freq:[.25,8],gain:[.2,.8],octaves:[1,12]}},computeRidgedMultifractal:{clamp:{freq:[.25,8],gain:[.2,.9],octaves:[2,14]}},computeRidgedMultifractal2:{clamp:{freq:[.25,8],gain:[.2,.9],octaves:[2,14]}},computeRidgedMultifractal3:{clamp:{freq:[.25,8],gain:[.2,.9],octaves:[2,14]}},computeRidgedMultifractal4:{clamp:{freq:[.25,8],gain:[.2,.9],octaves:[2,14]}},computeFBM:{clamp:{gain:[.2,.8],octaves:[2,10]}},computeFBM2:{clamp:{gain:[.2,.8],octaves:[2,10]}},computeFBM3:{clamp:{gain:[.2,.8],octaves:[2,10]}},computeVoronoiBM1:{clamp:{threshold:[0,1],edgeK:[0,64]}},computeVoronoiBM2:{clamp:{threshold:[0,1],edgeK:[0,64]}},computeVoronoiBM3:{clamp:{threshold:[0,1],edgeK:[0,64]}},computeCellular:{clamp:{threshold:[0,1]}},computeWorley:{clamp:{threshold:[0,1]}},computeAntiCellular:{clamp:{threshold:[0,1]}},computeAntiWorley:{clamp:{threshold:[0,1]}},computeSimplexFBM:{force:{turbulence:1},clamp:{warpAmp:[.1,2],freq:[.25,6]}},computeCurl2D:{force:{turbulence:1},clamp:{warpAmp:[.1,2],freq:[.25,6]}},computeCurlFBM2D:{force:{turbulence:1},clamp:{warpAmp:[.1,3]}},computeDomainWarpFBM1:{force:{turbulence:1},clamp:{warpAmp:[.1,3]}},computeDomainWarpFBM2:{force:{turbulence:1},clamp:{warpAmp:[.1,3]}},computeGaborAnisotropic:{clamp:{gaborRadius:[.5,6]}},computeFoamNoise:{force:{turbulence:1},clamp:{gain:[.5,.95]}}},N=128,be="toroidalDemo",K=new Map,I=[],$=Object.create(null);function Y(o){let t=String(o||""),e=Ve[t];if(e)return e;let i=t;return i.startsWith("compute")&&(i=i.slice(7)),i||t}function Fe(o,t){let e=Object.create(null),i=Array.isArray(o)?o:[],r=Math.max(0,t|0),a=Math.max(0,i.length-r);for(let n=0;n<a;n++)e[n]=Y(i[n]);return e}function Le(){return Object.keys($).map(o=>Number(o)).filter(o=>Number.isInteger(o)&&o>=0).sort((o,t)=>o-t)}function Ge(){let o=[];for(let t=0;t<I.length;t++){let e=I[t];typeof e!="string"||!e||e!=="clearTexture"&&o.push(t)}return o}function We(o){let t=I[o];return t&&Ie[String(t)]||null}function U(o,t,e,i){if(!Object.prototype.hasOwnProperty.call(o,t))return;let r=Number(o[t]);if(!Number.isFinite(r))return;let a=Number(e),n=Number(i);o[t]=Math.min(Math.max(r,a),n)}function _e(o,t){let e={...t},i=We(o);if(i&&i.clamp){let a=i.clamp;a.freq&&U(e,"freq",a.freq[0],a.freq[1]),a.gain&&U(e,"gain",a.gain[0],a.gain[1]),a.octaves&&U(e,"octaves",a.octaves[0],a.octaves[1]),a.threshold&&U(e,"threshold",a.threshold[0],a.threshold[1]),a.warpAmp&&U(e,"warpAmp",a.warpAmp[0],a.warpAmp[1]),a.gaborRadius&&U(e,"gaborRadius",a.gaborRadius[0],a.gaborRadius[1]),a.edgeK&&U(e,"edgeK",a.edgeK[0],a.edgeK[1])}if(i&&i.force)for(let[a,n]of Object.entries(i.force))e[a]=n;let r=K.get(o);if(r)for(let[a,n]of Object.entries(r))typeof n=="number"&&Number.isFinite(n)&&(e[a]=n);return e}function me(){let o=(a,n)=>{let l=document.getElementById(a);if(!l)return n;let s=Number(l.value);return Number.isFinite(s)?s:n},t=(a,n)=>{let l=o(a,n);return Number.isFinite(l)?Math.max(0,Math.floor(l)):n},e=Math.max(1,Math.floor(o("noise-seed",1234567890))),i=document.getElementById("noise-turbulence"),r=i&&i.checked?1:0;return{seed:e,zoom:o("noise-zoom",1),freq:o("noise-freq",1),octaves:Math.max(1,Math.floor(o("noise-octaves",8))),lacunarity:o("noise-lacunarity",2),gain:o("noise-gain",.5),xShift:o("noise-xShift",0),yShift:o("noise-yShift",0),zShift:o("noise-zShift",0),turbulence:r,seedAngle:o("noise-seedAngle",0),exp1:o("noise-exp1",1),exp2:o("noise-exp2",0),threshold:o("noise-threshold",.1),rippleFreq:o("noise-rippleFreq",10),time:o("noise-time",0),warpAmp:o("noise-warpAmp",.5),gaborRadius:o("noise-gaborRadius",4),terraceStep:o("noise-terraceStep",8),toroidal:0,voroMode:t("noise-voroMode",0),edgeK:o("noise-edgeK",0)}}function Ue(){let o=document.querySelectorAll('input[type="checkbox"][name="noise-type"]'),t=[];return o.forEach(e=>{if(e.checked){let i=Number(e.dataset.bit);Number.isInteger(i)&&t.push(i)}}),t}function H(){let o=document.getElementById("z-slice"),t=document.getElementById("z-slice-num"),e=0;return o?e=Number(o.value):t&&(e=Number(t.value)),Number.isFinite(e)||(e=0),e=Math.min(Math.max(Math.round(e),0),N-1),o&&String(o.value)!==String(e)&&(o.value=String(e)),t&&String(t.value)!==String(e)&&(t.value=String(e)),e}function re(o,t=null,e=null,i="contain"){o.style.display="block",o.style.margin="0",o.style.padding="0",o.style.border="0",o.style.outline="0",o.style.background="transparent",o.style.width=t!=null?`${t}px`:"100%",o.style.height=e!=null?`${e}px`:"100%",o.style.objectFit=i,o.style.objectPosition="center",o.style.imageRendering="crisp-edges",o.style.imageRendering="pixelated"}function de(o,t,e,i,r=null,a=null){let n=Math.max(1,e|0),l=Math.max(1,i|0);re(t,r,a);let s=!1;return(t.width!==n||t.height!==l)&&(t.width=n,t.height=l,s=!0),o&&typeof o.configureCanvas=="function"&&s&&o.configureCanvas(t),s}function Ye(o){o.style.display="grid",o.style.width="100%",o.style.height="100%",o.style.aspectRatio="1 / 1",o.style.gridTemplateColumns="repeat(3, 1fr)",o.style.gridTemplateRows="repeat(3, 1fr)",o.style.gap="0",o.style.padding="0",o.style.margin="0",o.style.border="0",o.style.lineHeight="0",o.style.fontSize="0",o.style.alignItems="stretch",o.style.justifyItems="stretch",o.style.alignContent="stretch",o.style.justifyContent="stretch",o.style.overflow="hidden",o.style.background="#000"}function Oe(){let o=document.getElementById("noise-canvas"),t=document.getElementById("view-stack");if(!o&&t&&(o=document.createElement("canvas"),o.id="noise-canvas",o.width=800,o.height=800,t.appendChild(o)),!o)throw new Error("Missing main preview canvas (#noise-canvas)");re(o,null,null,"contain");let e=document.getElementById("mosaic");if(!e)throw new Error("Missing #mosaic container");let i=9,r=Array.from(e.querySelectorAll("canvas"));if(r.length!==i){e.innerHTML="",r=[];for(let a=0;a<i;a++){let n=document.createElement("canvas");n.width=N,n.height=N,re(n,null,null,"fill"),e.appendChild(n),r.push(n)}}else r.forEach(a=>re(a,null,null,"fill"));return Ye(e),{mainCanvas:o,mosaicCanvases:r}}function De(o){return o.length?o.map(e=>$[e]||String(e)).join(", "):$[0]||"Perlin"}function Me(o){let t=document.getElementById(o);if(!t)throw new Error(`Missing #${o}`);return t}function Xe(){let o=Me("noise-type-list");o.innerHTML="";let t=Le();for(let e of t){let i=document.createElement("label"),r=document.createElement("input");r.type="checkbox",r.name="noise-type",r.dataset.bit=String(e),e===0&&(r.checked=!0),i.appendChild(r),i.appendChild(document.createTextNode(" "+($[e]||String(e)))),o.appendChild(i)}}function Ze(o){return(Array.isArray(o)?o:[]).filter(e=>typeof e=="string"&&/4D/.test(e)&&e!=="clearTexture").slice()}function He(o){let t=Me("toroidal-type-list");t.innerHTML="";let e=Ze(o),i=new Set(["computePerlin4D","computeWorley4D"]),r=!1;for(let a of e){let n=document.createElement("label"),l=document.createElement("input");l.type="checkbox",l.name="toroidal-type",l.dataset.entry=a;let s=I.indexOf(a);Number.isInteger(s)&&s>=0&&(l.dataset.bit=String(s)),i.has(a)&&(l.checked=!0,r=!0),n.appendChild(l),n.appendChild(document.createTextNode(" "+Y(a))),t.appendChild(n)}if(!r&&e.length){let a=t.querySelector('input[type="checkbox"][name="toroidal-type"]');a&&(a.checked=!0)}}function Z(){let o=document.querySelectorAll('input[type="checkbox"][name="toroidal-type"]'),t=[];if(o.forEach(e=>{if(!e.checked)return;let i=String(e.dataset.entry||"");if(!i)return;let r=Number(e.dataset.bit);Number.isInteger(r)||(r=I.indexOf(i)),Number.isInteger(r)||(r=-1),t.push({bit:r,entry:i})}),!t.length){let e=["computePerlin4D","computeWorley4D"];for(let i of e){if(!I.includes(i))continue;let r=I.indexOf(i);t.push({bit:r,entry:i})}}return t}function pe(o){let t=document.getElementById("mosaic-caption");if(!t)return;let e=Array.isArray(o)?o:[],i=e.length?e.map(r=>Y(r)).join(" + "):"None";t.textContent=`A single toroidal Z slice from a 4D volume. Modes: ${i}. Repeated in X and Y. Use the Z slice control to see different slices.`}function Ke(){let o=document.getElementById("override-mode");if(!o)return;o.innerHTML="";let t=Ge();for(let e of t){let i=I[e],r=document.createElement("option");r.value=String(e),r.textContent=`${e}: ${Y(i)}`,o.appendChild(r)}t.length&&(o.value=String(t[0]))}function le(o){let t=K.get(o)||{},e=(r,a)=>{let n=document.getElementById(r);if(!n)return;let l=t[a];n.value=typeof l=="number"&&Number.isFinite(l)?String(l):""},i=(r,a)=>{let n=document.getElementById(r);if(!n)return;let l=t[a];n.value=typeof l=="number"&&Number.isFinite(l)?String(l):""};e("ov-zoom","zoom"),e("ov-freq","freq"),e("ov-lacunarity","lacunarity"),e("ov-gain","gain"),e("ov-octaves","octaves"),i("ov-turbulence","turbulence"),e("ov-seedAngle","seedAngle"),e("ov-exp1","exp1"),e("ov-exp2","exp2"),e("ov-rippleFreq","rippleFreq"),e("ov-time","time"),e("ov-warp","warpAmp"),e("ov-threshold","threshold"),i("ov-voroMode","voroMode"),e("ov-edgeK","edgeK"),e("ov-gabor","gaborRadius"),e("ov-terraceStep","terraceStep"),e("ov-xShift","xShift"),e("ov-yShift","yShift"),e("ov-zShift","zShift")}function fe(){let o=document.getElementById("override-mode");if(!o)return;let t=Number(o.value);if(!Number.isInteger(t))return;let e=C=>{let T=document.getElementById(C);if(!T)return null;let q=String(T.value).trim();if(!q)return null;let S=Number(q);return Number.isFinite(S)?S:null},i=C=>{let T=document.getElementById(C);if(!T)return null;let q=String(T.value).trim();if(!q)return null;let S=Number(q);return Number.isFinite(S)?S:null},r={},a=e("ov-zoom"),n=e("ov-freq"),l=e("ov-lacunarity"),s=e("ov-gain"),f=e("ov-octaves"),p=i("ov-turbulence"),u=e("ov-seedAngle"),m=e("ov-exp1"),d=e("ov-exp2"),c=e("ov-rippleFreq"),v=e("ov-time"),x=e("ov-warp"),b=e("ov-threshold"),y=i("ov-voroMode"),h=e("ov-edgeK"),P=e("ov-gabor"),_=e("ov-terraceStep"),D=e("ov-xShift"),B=e("ov-yShift"),A=e("ov-zShift");a!==null&&(r.zoom=a),n!==null&&(r.freq=n),l!==null&&(r.lacunarity=l),s!==null&&(r.gain=s),f!==null&&(r.octaves=f),p!==null&&(r.turbulence=Math.max(0,Math.floor(p))),u!==null&&(r.seedAngle=u),m!==null&&(r.exp1=m),d!==null&&(r.exp2=d),c!==null&&(r.rippleFreq=c),v!==null&&(r.time=v),x!==null&&(r.warpAmp=x),b!==null&&(r.threshold=b),y!==null&&(r.voroMode=Math.max(0,Math.floor(y))),h!==null&&(r.edgeK=h),P!==null&&(r.gaborRadius=P),_!==null&&(r.terraceStep=_),D!==null&&(r.xShift=D),B!==null&&(r.yShift=B),A!==null&&(r.zShift=A),Object.keys(r).length?K.set(t,r):K.delete(t)}function ue(o){return typeof o=="string"&&/4d/i.test(o)}function $e(o,t=800){let e=document.getElementById("res-width"),i=document.getElementById("res-height"),r=(u,m)=>{let d=Number(u);return Number.isFinite(d)?Math.max(1,Math.floor(d)):m|0},a=o?.device?.limits||{},n=a.maxTextureDimension2D??8192,l=a.maxStorageTextureDimension2D??n,s=Math.min(n,l)|0,f=r(e?.value,t),p=r(i?.value,t);return f=Math.min(f,s),p=Math.min(p,s),e&&String(e.value)!==String(f)&&(e.value=String(f)),i&&String(i.value)!==String(p)&&(i.value=String(p)),{w:f,h:p}}function je(o,t,e,i){let a=Math.min(e,2048),n=Math.min(i,2048);de(o,t,a,n)}function j(o){let t=document.getElementById("preview-meta");t&&(t.textContent=o)}function O(o){let t=document.getElementById("preview-stats");t&&(t.textContent=o)}function Qe(o){if(!o)return;let t=o.resW|0,e=o.resH|0,i=Array.isArray(o.noiseBits)?o.noiseBits:[],a=i.some(l=>ue(I[l]))?" \xB7 toroidal(4D)":"",n=Math.max(t,e)|0;j(`Height field preview \xB7 ${t}\xD7${e} \xB7 world ${n}\xD7${n} \xB7 modes: ${De(i)}${a}`),typeof o.computeMs=="number"&&typeof o.blitMs=="number"?O(`GPU compute ${o.computeMs.toFixed(1)} ms \xB7 blit ${o.blitMs.toFixed(1)} ms`):O("")}async function ze(o,t,e={}){let i=e.updateUI!==!1,{w:r,h:a}=$e(o,800);je(o,t,r,a);let n=me();o.buildPermTable(n.seed|0);let l=Ue(),s=l.length?l:[0],f={getGradient:0,outputChannel:1,baseRadius:0,heightScale:1,useCustomPos:0,squareWorld:!0,worldMode:"crop"},p=performance.now();await o.computeToTexture(r,a,n,{...f,noiseChoices:["clearTexture"]});for(let b of s){let y=I[b],h=_e(b,n);h.toroidal=ue(y)?1:0,await o.computeToTexture(r,a,h,{...f,noiseChoices:[b]})}let u=performance.now(),m=o.getCurrentView(),d=performance.now();m&&o.renderTextureToCanvas(m,t,{layer:0,channel:0,preserveCanvasSize:!0,clear:!0});let c=performance.now(),v=u-p,x=c-d;if(i){let y=s.some(P=>ue(I[P]))?" \xB7 toroidal(4D)":"",h=Math.max(r,a)|0;j(`Height field preview \xB7 ${r}\xD7${a} \xB7 world ${h}\xD7${h} \xB7 modes: ${De(s)}${y}`),O(`GPU compute ${v.toFixed(1)} ms \xB7 blit ${x.toFixed(1)} ms`)}return{resW:r,resH:a,noiseBits:s,computeMs:v,blitMs:x}}function ce(o,t,e,i={}){if(!t)return 0;let r=N,n=(H()+.5)/r,l=Array.isArray(e)?e:[],s=l.length||9,f=performance.now();for(let u=0;u<s;u++){let m=l[u];m&&(de(o,m,N,N),o.renderTexture3DSliceToCanvas(t,m,{depth:r,zNorm:n,channel:0,chunk:0,preserveCanvasSize:!0,clear:!0}))}return performance.now()-f}async function we(o,t,e,i={}){let r=i.draw!==!1,a=i.updateUI!==!1,n=me();o.buildPermTable(n.seed|0);let l={...n,toroidal:1},s=Z();pe(s.map(d=>d.entry));let f=performance.now(),p=await o.computeToTexture3D(N,N,N,l,{noiseChoices:["clearTexture"],outputChannel:1,id:be});for(let d of s){let c=d.bit,v=d.entry,x=Number.isInteger(c)&&c>=0?_e(c,l):{...l};x.toroidal=1,p=await o.computeToTexture3D(N,N,N,x,{noiseChoices:[v],outputChannel:1,id:be})}let u=performance.now();e.lastToroidalVolumeView=p,e.lastToroidalComputeMs=u-f;let m=0;if(r&&(m=ce(o,p,t)),a){let d=s.length?s.map(x=>Y(x.entry)).join(" + "):"None";j(`Toroidal tiles \xB7 ${N}\xB3 \xB7 modes: ${d} \xB7 Z slice: ${H()}`);let c=e.lastToroidalComputeMs.toFixed(1),v=m.toFixed(1);O(r?`GPU volume compute ${c} ms \xB7 slice blit ${v} ms`:`GPU volume compute ${c} ms`)}return{computeMs:e.lastToroidalComputeMs,sliceBlitMs:m}}function G(){let o=document.getElementById("view-tab-tileset");return o&&o.checked?"tileset":"main"}async function Je(){let o=document.getElementById("preview-stats");if(!navigator.gpu){console.error("WebGPU not available in this browser."),o&&(o.textContent="WebGPU not available in this browser.");return}let t=await navigator.gpu.requestAdapter();if(!t){console.error("Failed to get GPU adapter."),o&&(o.textContent="Failed to get GPU adapter.");return}let e=await t.requestDevice({requiredLimits:{maxBufferSize:t.limits.maxBufferSize}}),i=new ie(e,e.queue);I=Array.isArray(i.entryPoints)?i.entryPoints.slice():[],$=Fe(I,ke),Xe(),He(I),Ke();let{mainCanvas:r,mosaicCanvases:a}=Oe();i.configureCanvas(r),a.forEach(g=>i.configureCanvas(g));let n=document.getElementById("override-mode");if(n){let g=Number(n.value);Number.isInteger(g)&&le(g)}let l={lastToroidalVolumeView:null,lastToroidalComputeMs:0,lastMainInfo:null},s={main:!0,tileset:!0},f=!1,p=!1,u=()=>{p=!0,!f&&(f=!0,requestAnimationFrame(async()=>{p=!1;let g=G();try{if(g==="main")s.main?(s.main=!1,l.lastMainInfo=await ze(i,r,{updateUI:!0})):Qe(l.lastMainInfo);else if(s.tileset||!l.lastToroidalVolumeView)s.tileset=!1,await we(i,a,l,{draw:!0,updateUI:!0});else{let z=ce(i,l.lastToroidalVolumeView,a),E=Z(),V=E.length?E.map(J=>Y(J.entry)).join(" + "):"None";j(`Toroidal tiles \xB7 ${N}\xB3 \xB7 modes: ${V} \xB7 Z slice: ${H()}`),O(`GPU volume compute ${l.lastToroidalComputeMs.toFixed(1)} ms \xB7 slice blit ${z.toFixed(1)} ms`)}}catch(z){console.error(z),o&&(o.textContent=String(z))}f=!1,p&&u()}))},m=(g=!0)=>{s.main=!0,g&&G()==="main"&&u()},d=(g=!0)=>{s.tileset=!0,g&&G()==="tileset"&&u()},c=()=>{s.main=!0,s.tileset=!0,u()};["ov-zoom","ov-freq","ov-lacunarity","ov-gain","ov-octaves","ov-turbulence","ov-seedAngle","ov-exp1","ov-exp2","ov-rippleFreq","ov-time","ov-warp","ov-threshold","ov-voroMode","ov-edgeK","ov-gabor","ov-terraceStep","ov-xShift","ov-yShift","ov-zShift"].forEach(g=>{let z=document.getElementById(g);z&&z.addEventListener("change",()=>{fe(),c()})}),n&&n.addEventListener("change",()=>{let g=Number(n.value);Number.isInteger(g)&&le(g)});let x=document.getElementById("ov-clear");x&&x.addEventListener("click",()=>{let g=document.getElementById("override-mode");if(!g)return;let z=Number(g.value);Number.isInteger(z)&&(K.delete(z),le(z),c())});let b=document.getElementById("render-btn");b&&b.addEventListener("click",()=>{G()==="main"?m(!0):d(!0)});let y=document.getElementById("apply-res");y&&y.addEventListener("click",()=>{m(!0)}),["noise-seed","noise-zoom","noise-freq","noise-octaves","noise-lacunarity","noise-gain","noise-xShift","noise-yShift","noise-zShift","noise-voroMode","noise-threshold","noise-edgeK","noise-seedAngle","noise-turbulence","noise-time","noise-warpAmp","noise-gaborRadius","noise-terraceStep","noise-exp1","noise-exp2","noise-rippleFreq"].forEach(g=>{let z=document.getElementById(g);z&&(z.addEventListener("input",()=>{s.main=!0,s.tileset=!0,u()}),z.addEventListener("change",()=>{s.main=!0,s.tileset=!0,u()}))});let P=document.getElementById("noise-type-list");P&&P.addEventListener("change",g=>{let z=g.target;!z||z.name!=="noise-type"||m(!0)});let _=document.getElementById("toroidal-type-list");_&&_.addEventListener("change",g=>{let z=g.target;!z||z.name!=="toroidal-type"||(pe(Z().map(E=>E.entry)),d(!0))});let D=document.getElementById("z-slice"),B=document.getElementById("z-slice-num"),A=()=>{if(G()!=="tileset"||!l.lastToroidalVolumeView)return;let g=ce(i,l.lastToroidalVolumeView,a);j(`Toroidal tiles \xB7 ${N}\xB3 \xB7 Z slice: ${H()}`),O(`GPU volume compute ${l.lastToroidalComputeMs.toFixed(1)} ms \xB7 slice blit ${g.toFixed(1)} ms`)};D&&D.addEventListener("input",()=>{let g=Number(D.value);B&&(B.value=String(g)),A()}),B&&B.addEventListener("change",()=>{let g=Number(B.value);Number.isFinite(g)||(g=0),g=Math.min(Math.max(Math.round(g),0),N-1),B.value=String(g),D&&(D.value=String(g)),A()});let C=document.getElementById("view-tab-preview"),T=document.getElementById("view-tab-tileset");C&&C.addEventListener("change",()=>{u()}),T&&T.addEventListener("change",()=>{u()});function q(g,z){let E=URL.createObjectURL(g),V=document.createElement("a");V.href=E,V.download=z,document.body.appendChild(V),V.click(),V.remove(),URL.revokeObjectURL(E)}function S(g){return String(g||"").trim().replace(/\s+/g,"_").replace(/[^a-zA-Z0-9._-]+/g,"").slice(0,120)}function R(){let g=document.querySelector('input[type="radio"][name="export-bg"]:checked'),z=String(g?.value||"transparent");return z==="black"||z==="white"||z==="transparent"?z:"transparent"}function w(g){let z=R();return g&&typeof g.setExportBackground=="function"&&g.setExportBackground(z),z}function M(g){w(g),document.querySelectorAll('input[type="radio"][name="export-bg"]').forEach(E=>{E.addEventListener("change",()=>{w(g)})})}async function L(){fe(),await we(i,a,l,{draw:G()==="tileset",updateUI:G()==="tileset"})}let F=document.getElementById("download-main");F&&F.addEventListener("click",async()=>{try{fe();let g=w(i),z=Number(document.getElementById("res-width")?.value)||800,E=Number(document.getElementById("res-height")?.value)||800;de(i,r,z,E),await ze(i,r,{updateUI:G()==="main"});let V=await i.exportCurrent2DToPNGBlob(z,E,{layer:0,channel:0,background:g});q(V,"noise-main.png")}catch(g){console.error("download-main failed:",g),o&&(o.textContent="Export main PNG failed: "+g)}});let W=document.getElementById("download-tile");W&&W.addEventListener("click",async()=>{try{let g=w(i);if(await L(),!l.lastToroidalVolumeView){console.warn("No toroidal volume available for export");return}let z=N,E=N,V=N,X=(H()+.5)/V,ee=await i.export3DSliceToPNGBlob(l.lastToroidalVolumeView,z,E,{depth:V,zNorm:X,channel:0,chunk:0,background:g});q(ee,"noise-tile.png")}catch(g){console.error("download-tile failed:",g),o&&(o.textContent="Export tile PNG failed: "+g)}});async function Q(g,z){if(!z)return;let E=w(g);if(await L(),!z.lastToroidalVolumeView){console.warn("No toroidal volume available for tileset export");return}let V=me(),J=Z().map(Ae=>Ae.entry),X=16,ee=N,ge=N,te=N,ve=Math.ceil(te/X),Be=await g.export3DTilesetToPNGBlob(z.lastToroidalVolumeView,ee,ge,{depth:te,channel:0,chunk:0,tilesAcross:X,tilesDown:ve,startSlice:0,sliceCount:te,background:E}),Se=S(J.map(Y).join("+"))||"tileset",Pe=S(V.seed),Ce=`noise-tileset_${Se}_seed${Pe}_${ee}x${ge}_z${te}_${X}x${ve}.png`;q(Be,Ce)}let k=document.getElementById("download-tileset");k&&k.addEventListener("click",async()=>{try{await Q(i,l)}catch(g){console.error("download-tileset failed:",g),o&&(o.textContent="Export tileset failed: "+g)}}),M(i),pe(Z().map(g=>g.entry)),u()}document.addEventListener("DOMContentLoaded",()=>{Je().catch(o=>console.error(o))});})();
